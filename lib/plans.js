...
export const PLANS = [
  {
    ...
    includedPerms: [
      // Operations
      // 'orderKitchen' was split into 'orders' (waiter station) and
      // 'kitchenStation' (KDS) on 2026-06-03 — see lib/permissions.js.
      'tables', 'newOrder', 'orders', 'kitchenStation', 'kitchen', 'waiter', 'payments',
      // Overview
      ...
      // Setup (settings + manageRoles are foundational — every restaurant
      // needs them regardless of tier; they aren't a paywall lever.)
      'qrcode', 'settings', 'manageRoles',
    ],
    prices: {
      monthly:    M_STARTER,
  ...
  },
  {
    ...
    maxStaff:      8,
    includedPerms: [
      // All Starter perms (incl. foundational kitchen/waiter/settings/roles)
      // 'orderKitchen' was split into 'orders' (waiter station) and
      // 'kitchenStation' (KDS) on 2026-06-03 — see lib/permissions.js.
      'tables', 'newOrder', 'orders', 'kitchenStation', 'kitchen', 'waiter', 'payments',
      'analytics', 'reports', 'dayClose', 'activity',
  ...
  },
  {
    ...
    maxStaff:      UNLIMITED,
    includedPerms: [
      // All Growth perms (incl. foundational kitchen/waiter/settings/roles)
      // 'orderKitchen' was split into 'orders' (waiter station) and
      // 'kitchenStation' (KDS) on 2026-06-03 — see lib/permissions.js.
      'tables', 'newOrder', 'orders', 'kitchenStation', 'kitchen', 'waiter', 'payments',
      'analytics', 'reports', 'dayClose', 'activity',
...
// Phase F — founding-partner price override. A restaurant doc MAY carry
// `foundingPricing` that pins specific {inr, paise} for some/all
// (plan × period) combinations. If present, it takes precedence over the
// plan's standard price for THIS restaurant — used to honor the "first
// 50 partners get locked-in pricing for life" promise on the landing
// page. Combos NOT in the override fall through to the standard pricing.
//
// Shape (only the combos that need locking need entries):
//   foundingPricing: {
//     starter: { monthly: { inr, paise }, annual: { inr, paise } },
//     growth:  { ... },
//     pro:     { ... }
//   }
export function getEffectivePrice(planId, period = 'monthly', restaurant = null) {
  const override = restaurant?.foundingPricing?.[normalizePlanId(planId)]?.[period];
  if (override && Number.isFinite(Number(override.inr))) return Number(override.inr);
  return getPrice(planId, period);
}
export function getEffectivePriceInPaise(planId, period = 'monthly', restaurant = null) {
  const override = restaurant?.foundingPricing?.[normalizePlanId(planId)]?.[period];
  if (override && Number.isFinite(Number(override.paise))) return Number(override.paise);
  return getPriceInPaise(planId, period);
}

// True iff the restaurant has any founding-pricing override at all.
// Used by the subscription UI to show a "Founding partner pricing"
// badge on plan tiles that are overridden.
export function hasFoundingPricing(restaurant) {
  return !!restaurant?.foundingPricing && Object.keys(restaurant.foundingPricing).length > 0;
}

// ── Cap helpers (Phase G — fixes the "0/60 on Growth" bug) ─────────
// EFFECTIVE cap for a restaurant on a given plan field. Always reads
// from the canonical plan in lib/plans.js — NEVER the stale value
// possibly written on the restaurant doc by an older verify.js run.
// This way existing restaurants pick up new tier caps the moment
// lib/plans.js changes, with zero migration.
//
// The doc still stores maxItems/maxStorageMB/etc. (verify.js writes
// them on each payment) but those are historical record only, not
// the display or enforcement source. Per-restaurant custom caps
// (rare — superadmin tooling) can ship later as `customCaps: {...}`
// that this helper checks first.
export function planCap(restaurantOrPlanId, capName) {
  const planId = typeof restaurantOrPlanId === 'string'
    ? restaurantOrPlanId
    : restaurantOrPlanId?.plan;
  const plan = getPlan(planId);
  const v = plan[capName];
  return typeof v === 'number' ? v : 0;
}

// Format a cap for display — renders the UNLIMITED sentinel as a
// human-readable word instead of "99999". Use everywhere a cap is
// shown in the UI.
...
// /admin/petpooja-pos page, the /api/petpooja/* endpoints, and
