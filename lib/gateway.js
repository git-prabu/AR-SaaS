...
import * as razorpay from './gatewayProviders/razorpay';

const PROVIDERS = { paytm, razorpay };
...
export async function setGatewayConfig(restaurantId, config) {
  ...
  // merge:true so partial saves (e.g. only the autoConfirm subkey) don't
  // wipe the other half of the config. The /api/payment/config endpoint
  // always sends a full sanitised object so we don't lose fields the
  // admin DID edit either.
  await adminDb.doc(`restaurants/${restaurantId}/private/gateway`).set(config, { merge: true });
  await adminDb.doc(`restaurants/${restaurantId}`).set(
    {
      gatewayActive: !!config.isActive,
      gatewayProvider: config.provider || null,
      // Mirror autoConfirm active-state too so the customer page can
      // read it via the public restaurant doc (the customer doesn't
      // need the secrets, just the boolean "is this enabled").
      autoConfirmActive: !!(config.autoConfirm?.isActive && config.autoConfirm?.provider && config.autoConfirm.provider !== 'none'),
      autoConfirmProvider: config.autoConfirm?.provider || null,
      // Mirror preview-mode flag too — customer page ORs it with
      // autoConfirmActive to render the waiting UI for demo purposes.
      autoConfirmPreview: !!config.autoConfirm?.previewMode,
    },
    { merge: true }
  );
}
