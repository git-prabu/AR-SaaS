...
// Staff Storage — tied to staffApp so a staff member's auth token rides on
// image uploads (storage.rules `allow write: if isAuth()` then passes). Used
// by RBAC-converted catalog pages (Menu Items / Add Items) when a staffer
// uploads a dish photo. Uploading through the admin `storage` from a staff
// session would be unauthenticated → storage/unauthorized.
