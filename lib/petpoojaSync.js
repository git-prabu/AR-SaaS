...
//
// Logging policy (May 5): when we refuse work we emit a single-line
// console.warn so the daily Petpooja cron + ad-hoc syncs leave a
// breadcrumb trail. Without it, restaurants that downgrade off Pro or
// flip to standalone silently get every cron tick skipped — and we
// only find out when a user complains. plan-not-eligible is the most
// common case and the most useful to log.
async function loadAndGate(restaurantId) {
  ...
  if (!canUsePetpoojaIntegration(restaurant)) {
    console.warn(`[petpoojaSync] skip rid=${restaurantId} reason=plan-not-eligible plan=${restaurant.plan || 'unknown'}`);
    return { ok: false, reason: 'plan-not-eligible', restaurant };
  }
  if (restaurant.posMode !== 'petpooja_hybrid') {
    console.warn(`[petpoojaSync] skip rid=${restaurantId} reason=not-in-hybrid-mode posMode=${restaurant.posMode || 'standalone'}`);
    return { ok: false, reason: 'not-in-hybrid-mode', restaurant };
  }
  const config = restaurant.petpoojaConfig;
  if (!config?.restID || !config?.apiKey) {
    console.warn(`[petpoojaSync] skip rid=${restaurantId} reason=missing-credentials`);
...
// ── 3. Disconnect ────────────────────────────────────────────────────
// Flips the restaurant back to standalone mode. Petpooja-mirrored items
// are HIDDEN from the customer menu (isActive=false) so a downgraded
// restaurant doesn't keep showing items they can no longer manage.
// petpoojaItemId is kept on the docs so a re-connect cleanly resumes
// syncing the same items. Admin can re-activate or delete from
// /admin/items if they want to keep them as standalone items.
//
// `reason` is recorded for audit ('user-requested' | 'plan-downgrade').
export async function disconnect(restaurantId, reason = 'user-requested') {
  ...
  // Deactivate Petpooja-mirrored items so they vanish from the customer
  // menu page (which filters on isActive). Batched in chunks of 450 to
  // stay under Firestore's 500-op batch limit. Best-effort — failures
  // here don't roll back the disconnect itself, just leave a log.
  try {
    const itemsSnap = await adminDb.collection(`restaurants/${restaurantId}/menuItems`).get();
    const targets = itemsSnap.docs.filter(d => !!d.data().petpoojaItemId);
    let hidden = 0;
    for (let i = 0; i < targets.length; i += 450) {
      ...
      for (const d of chunk) {
        batch.update(d.ref, {
          isActive: false,
          lastModifiedBy: `petpooja-disconnect:${reason}`,
    ...
    await appendLog(restaurantId, 'disconnect', { ok: true, reason, hiddenItems: hidden });
  } catch (err) {
    console.warn('[petpoojaSync.disconnect] hide-items failed:', err?.message);
    await appendLog(restaurantId, 'disconnect', {
      ok: true, reason, hideItemsError: err?.message || 'unknown',
    });
  }
  return { ok: true };
}
...
export async function applyMenu(restaurantId, menu, source = 'menu-sync') {
  ...
  // Sequential commit so we can attribute failures to a specific
  // batch index. If any batch throws, we surface a partial-success
  // result to the caller (instead of letting the throw propagate and
  // leaving the menu half-mirrored with no audit trail).
  let committed = 0;
  for (let i = 0; i < batches.length; i++) {
    try {
      ...
      const failureMsg = `batch ${i + 1}/${batches.length} failed: ${err.message}`;
      await adminDb.doc(`restaurants/${restaurantId}`).update({
        'petpoojaConfig.syncErrorCount': admin.firestore.FieldValue.increment(1),
        'petpoojaConfig.lastSyncError':  failureMsg,
      });
      await appendLog(restaurantId, source, {
        ok: false,
        error: failureMsg,
        committedBatches: committed,
        totalBatches: batches.length,
      });
      return { ok: false, error: failureMsg, committedBatches: committed, totalBatches: batches.length };
    }
  }
...
export async function pushOrder(restaurantId, orderId) {
  ...
  const callbackUrl = process.env.PETPOOJA_CALLBACK_URL
    || (process.env.NEXT_PUBLIC_SITE_URL || 'https://HaloHelm.vercel.app') + '/api/petpooja/callback';
