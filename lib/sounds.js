...
// Prime SpeechSynthesisUtterance for iOS Safari. Without this, the
// first speak() call from a non-gesture context (e.g. inside a
// Firestore onSnapshot callback) silently fails on iOS because the
// browser requires user activation for the first utterance. We fire a
// zero-volume utterance during the gesture so subsequent calls work.
function unlockVoice() {
  if (typeof window === 'undefined') return;
  if (!('speechSynthesis' in window)) return;
  try {
    // Tiny zero-volume utterance — inaudible to the user, but enough
    // to satisfy iOS's user-activation requirement.
    const u = new SpeechSynthesisUtterance(' ');
    u.volume = 0;
    u.rate = 1;
    window.speechSynthesis.speak(u);
    // Some iOS versions queue the silent utterance forever in
    // 'pending' state if we don't kick the engine. cancel() right
    // after speak() flushes the queue without dropping the unlock.
    window.speechSynthesis.cancel();
  } catch {}
}

// Auto-unlock on the first user gesture anywhere in the page.
// Without this, only pages that explicitly call unlockSound() inside
// a click handler can play sounds — owner reported that notification
// chimes weren't firing on most pages because the AudioContext was
// stuck in 'suspended' state, since the user might never tap the
// sound-toggle button (it was the only handler wired up).
//
// This handler runs once per page load (idempotent — capture phase,
// once: true), resumes the context, primes speechSynthesis, then
// unregisters itself.
