...
export async function getReservations(restaurantId) {
  const snap = await getDocs(
    query(collection(db, 'restaurants', restaurantId, 'reservations'), orderBy('date', 'asc'))
  );
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

export async function updateReservation(restaurantId, resId, data) {
  return updateDoc(doc(db, 'restaurants', restaurantId, 'reservations', resId), withActor(data));
}

export async function deleteReservation(restaurantId, resId) {
  return deleteDoc(doc(db, 'restaurants', restaurantId, 'reservations', resId));
}
...
// ── Vendors ───────────────────────────────────────────────────
export async function getVendors(restaurantId) {
  const snap = await getDocs(collection(db, 'restaurants', restaurantId, 'vendors'));
  return snap.docs
    .map(d => ({ id: d.id, ...d.data() }))
...
export async function createVendor(restaurantId, data) {
  return addDoc(collection(db, 'restaurants', restaurantId, 'vendors'), withActor({
...
export async function updateVendor(restaurantId, vendorId, data) {
  return updateDoc(
    doc(db, 'restaurants', restaurantId, 'vendors', vendorId),
    withActor({ ..._cleanVendor(data), updatedAt: serverTimestamp() })
  );
}

export async function deleteVendor(restaurantId, vendorId) {
  return deleteDoc(doc(db, 'restaurants', restaurantId, 'vendors', vendorId));
}

// ── Expenses ──────────────────────────────────────────────────
const _EXPENSE_MODES = ['cash', 'card', 'upi', 'credit'];

export async function getExpenses(restaurantId, max = 500) {
...
export async function createExpense(restaurantId, data) {
  return addDoc(collection(db, 'restaurants', restaurantId, 'expenses'), withActor({
...
export async function deleteExpense(restaurantId, expenseId) {
  return deleteDoc(doc(db, 'restaurants', restaurantId, 'expenses', expenseId));
}
...
export async function getPurchaseOrders(restaurantId, max = 200) {
  const snap = await getDocs(query(
    collection(db, 'restaurants', restaurantId, 'purchaseOrders'),
    orderBy('date', 'desc'), limit(max)
  ));
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

export async function createPurchaseOrder(restaurantId, data) {
  const totals = _poTotals(data);
  return addDoc(collection(db, 'restaurants', restaurantId, 'purchaseOrders'), withActor({
...
export async function updatePurchaseOrder(restaurantId, poId, data) {
  ...
  return updateDoc(doc(db, 'restaurants', restaurantId, 'purchaseOrders', poId), withActor(payload));
}

export async function deletePurchaseOrder(restaurantId, poId) {
  return deleteDoc(doc(db, 'restaurants', restaurantId, 'purchaseOrders', poId));
}

// Move a PO along its lifecycle. 'received' also stamps receivedAt so the
// list can show when stock actually landed.
export async function setPurchaseOrderStatus(restaurantId, poId, status) {
  if (!_PO_STATUSES.includes(status)) throw new Error('invalid PO status');
  const patch = { status };
  if (status === 'received') patch.receivedAt = serverTimestamp();
  return updateDoc(doc(db, 'restaurants', restaurantId, 'purchaseOrders', poId), withActor(patch));
}
...
export async function recordPurchaseOrderExpense(restaurantId, po, paymentMode = 'credit') {
  ...
  try {
    await updateDoc(
      doc(db, 'restaurants', restaurantId, 'purchaseOrders', po.id),
