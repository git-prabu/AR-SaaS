...
export async function createOrder(restaurantId, data) {
  ...
  try {
    orderNumber = await runTransaction(db, async (txn) => {
    ...
    // Fallback: if the transaction fails for any reason, still create the order so the
    // customer isn't blocked. The order will have no orderNumber; the backfill script can
    // assign one later. Log so it's visible.
    console.error('[createOrder] transaction failed, falling back to non-numbered order:', err);
