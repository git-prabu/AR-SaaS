...
// ───────────────────────────────────────────────────────────────
// STAFF ACTIVITY LOG (12 Jun 2026 — owner request: "see what was
// done by them"). lastModifiedBy only keeps the LAST writer per doc;
// this is the append-only history that powers the per-staff activity
// timeline on /admin/staff.
//
// One doc per action at restaurants/{rid}/staffActivity/{autoId}:
//   { at, actorKind: 'staff'|'admin', staffId|null, actorName,
//     action, refType, refId, tableNumber, amount, detail }
//
// Actions (industry-standard accountability set — what Petpooja-class
// POS "user-wise" reports track): login · order_placed ·
// order_cancelled · order_served · item_ready · item_served ·
// payment_marked · table_settled · call_resolved · bill_printed ·
// kot_printed.
//
// Design rules:
//   - FIRE-AND-FORGET: logging must never slow down or break the
//     operation being logged. Callers don't await; errors swallow.
//   - STAFF/ADMIN ONLY: anonymous customer writes (actor 'public')
//     are deliberately NOT logged — this is a staff accountability
//     trail, not a request log.
//   - Identity comes from the staff session blob (richer than the
//     bare auth UID: id + display name); owner falls back to 'Owner'.
// ───────────────────────────────────────────────────────────────
function activityActor() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (s?.staffId) {
        return { actorKind: 'staff', staffId: s.staffId, actorName: s.name || s.username || 'Staff' };
      }
    }
  } catch {}
  const uid = actorUid();
  if (uid && uid !== 'public' && uid !== 'system') {
    return { actorKind: 'admin', staffId: null, actorName: 'Owner' };
  }
  return null;
}

export function logStaffActivity(firestore, restaurantId, entry = {}) {
  try {
    const actor = activityActor();
    if (!actor || !restaurantId || !entry.action) return; // public/customer → skip
    addDoc(collection(firestore || db, 'restaurants', restaurantId, 'staffActivity'), {
      at: serverTimestamp(),
      ...actor,
      action: String(entry.action),
      refType: entry.refType || null,
      refId: entry.refId || null,
      tableNumber: entry.tableNumber != null ? String(entry.tableNumber) : null,
      amount: entry.amount != null ? Number(entry.amount) || 0 : null,
...
export async function getRestaurantById(id, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDoc(doc(fs, 'restaurants', id));
...
export async function createRestaurant(data) {
  // Defaults derived from the canonical Starter plan in lib/plans.js
  // (single source of truth). Caller can still override via `data`,
  // and the public-signup path uses pages/api/restaurant/create.js
  // (admin SDK) for server-trusted plan values.
  ...
  return addDoc(collection(db, 'restaurants'), {
    ...
    maxARModels:   starter.maxARModels,
    maxStorageMB:  starter.maxStorageMB,
    maxStaff:      starter.maxStaff,
...
export async function updateRestaurant(id, data, opts = {}) {
  const fs = opts.db || db;
  return updateDoc(doc(fs, 'restaurants', id), withActor(data));
}

// ─── Storage accounting ───────────────────────────────────────
// Atomic increment of the restaurant's storageUsedMB metric. Pass a
// positive deltaMB on upload, a negative one on delete/optimize. Uses
// Firestore's atomic increment() so concurrent uploads don't race a
// stale read — the old pattern in pages/superadmin/requests.js did
// `updateRestaurant({ storageUsedMB: prev + sizeMB })`, which lost
// writes when two uploads landed at the same time.
//
// storageUsedMB is FORWARD-TRACKED ONLY: legacy menu items uploaded
// before this helper existed have no stored imageSize/modelSize, so
// deleting them won't decrement (and uploading them never incremented).
// Net change for legacy items is therefore zero, which is consistent.
export async function bumpStorageUsed(restaurantId, deltaMB, opts = {}) {
  const fs = opts.db || db;
  const n = Number(deltaMB);
  if (!Number.isFinite(n) || n === 0) return;
  return updateDoc(doc(fs, 'restaurants', restaurantId), {
    storageUsedMB: increment(parseFloat(n.toFixed(3))),
  });
}
...
export async function getAllMenuItems(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(
    collection(fs, 'restaurants', restaurantId, 'menuItems')
  );
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function updateMenuItem(restaurantId, itemId, data, opts = {}) {
  const fs = opts.db || db;
  return updateDoc(
    doc(fs, 'restaurants', restaurantId, 'menuItems', itemId),
    withActor({ ...data, updatedAt: serverTimestamp() })
  );
}

export async function deleteMenuItem(restaurantId, itemId, opts = {}) {
  const fs = opts.db || db;
  const ref = doc(fs, 'restaurants', restaurantId, 'menuItems', itemId);
  // Pre-read the item so we can release its accounted storage on the
  // restaurant doc. Legacy items without imageSize/modelSize contribute
  // zero — see the comment on bumpStorageUsed for the forward-tracking
  // policy.
  let released = 0;
  try {
    ...
    if (snap.exists()) {
      const it = snap.data();
      released += Number(it.imageSize) || 0;
      released += Number(it.modelSize) || 0;
    }
  } catch { /* non-fatal: deletion still proceeds */ }
  await deleteDoc(ref);
  if (released > 0) {
    try { await bumpStorageUsed(restaurantId, -released, opts); } catch { /* best-effort */ }
  }
}

// ─── Requests ─────────────────────────────────────────────────

export async function getRequests(restaurantId, status = null, opts = {}) {
  const fs = opts.db || db;
  // Avoid composite indexes — filter/sort client-side
  const snap = await getDocs(
    collection(fs, 'restaurants', restaurantId, 'requests')
  );
...
export async function deleteRequest(restaurantId, requestId, opts = {}) {
  const fs = opts.db || db;
  return deleteDoc(doc(fs, 'restaurants', restaurantId, 'requests', requestId));
}

// New flow: publish menu item immediately (no AR), then create request for 3D upload
export async function submitRequestAndPublish(restaurantId, data, restaurant, opts = {}) {
  const fs = opts.db || db;
  ...
  const reqRef = await addDoc(collection(fs, 'restaurants', restaurantId, 'requests'), {
    ...data,
    status: 'pending',
    createdAt: serverTimestamp(),
    reviewedAt: null,
  ...
  await setDoc(doc(fs, 'restaurants', restaurantId, 'menuItems', reqRef.id), {
    name: data.name,
    ...
    imageSize: Number(data.imageSize) || 0,   // MB — release on delete
    modelURL: null,
    modelSize: 0,                              // populated by superadmin AR approval
    arReady: false,
  ...
  await updateDoc(doc(fs, 'restaurants', restaurantId), {
    itemsUsed: increment(1),
    ...stamp,
  });

  // 4. Bump storage usage for the uploaded image (if any). Best-effort —
  // a failure here doesn't unwind the menu item itself; the metric just
  // stays slightly under for this item, which the next item delete will
  // also under-decrement, so net drift is zero.
  const imgMB = Number(data.imageSize) || 0;
  if (imgMB > 0) {
    try { await bumpStorageUsed(restaurantId, imgMB, opts); } catch { /* best-effort */ }
  }
...
export async function getAnalytics(restaurantId, days = 30, startDate = null, endDate = null, opts = {}) {
  const fs = opts.db || db;
  ...
  const snap = await getDocs(
    collection(fs, 'restaurants', restaurantId, 'analytics')
  );
  const all = snap.docs
    .map(d => d.data())
...
export async function getAllOffers(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(
    collection(fs, 'restaurants', restaurantId, 'offers')
  );
  return snap.docs
    .map(d => ({ id: d.id, ...d.data() }))
...
export async function createOffer(restaurantId, data, opts = {}) {
  const fs = opts.db || db;
  return addDoc(collection(fs, 'restaurants', restaurantId, 'offers'), withActor({
    ...data,
    createdAt: serverTimestamp(),
  }));
}

export async function updateOffer(restaurantId, offerId, data, opts = {}) {
  const fs = opts.db || db;
  return updateDoc(doc(fs, 'restaurants', restaurantId, 'offers', offerId), withActor({
    ...data,
    updatedAt: serverTimestamp(),
  }));
}

export async function deleteOffer(restaurantId, offerId, opts = {}) {
  const fs = opts.db || db;
  return deleteDoc(doc(fs, 'restaurants', restaurantId, 'offers', offerId));
}
...
export async function getAreas(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(
    query(collection(fs, 'restaurants', restaurantId, 'areas'), orderBy('sortOrder', 'asc'))
  );
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function getTables(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(
    query(collection(fs, 'restaurants', restaurantId, 'tables'), orderBy('sortOrder', 'asc'))
  );
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function getOrCreateCaptainBill(restaurantId, tableCode, opts = {}) {
  const fs = opts.db || db;
  const code = String(tableCode || '').trim();
  if (!code) throw new Error('table code required');
  const sessionRef = doc(fs, 'restaurants', restaurantId, 'tableSessions', code);
  const sessionSnap = await getDoc(sessionRef);
  const currentBillId = sessionSnap.exists() ? sessionSnap.data()?.currentBillId : null;
  if (currentBillId) {
    const billSnap = await getDoc(doc(fs, 'restaurants', restaurantId, 'tableBills', currentBillId));
    if (billSnap.exists() && billSnap.data()?.status === 'open') return currentBillId;
  }
  const nowISO = new Date().toISOString();
  const billRef = await addDoc(collection(fs, 'restaurants', restaurantId, 'tableBills'), withActor({
    tableNumber: code,
    status: 'open',
    openedAt: nowISO,
    closedAt: null,
...
export async function markKotPrinted(restaurantId, billId, opts = {}) {
  if (!billId) return;
  const fs = opts.db || db;
  const res = await updateDoc(
    doc(fs, 'restaurants', restaurantId, 'tableBills', billId),
    withActor({ kotPrintedAt: serverTimestamp() })
  );
  logStaffActivity(fs, restaurantId, { action: 'kot_printed', refType: 'bill', refId: billId });
  return res;
}

// Phase 1b — stamp a bill as bill-printed → Table View green "Printed".
export async function markBillPrinted(restaurantId, billId, opts = {}) {
  if (!billId) return;
  const fs = opts.db || db;
  const res = await updateDoc(
    doc(fs, 'restaurants', restaurantId, 'tableBills', billId),
  ...
  // Bill reprints are an industry-standard fraud-watch signal (reprint
  // + pocket the cash) — every print lands in the trail.
  logStaffActivity(fs, restaurantId, { action: 'bill_printed', refType: 'bill', refId: billId });
  return res;
}

// Bill v2 (12 Jun 2026) — formal invoice number series.
//
// Every reference bill the owner collected (Petpooja et al) carries a
// "Bill No." that's a RUNNING per-restaurant series, separate from the
// per-day token/orderNumber. We assign it lazily at FIRST PRINT — the
// invoice is "issued" when it's printed, so never-billed tables don't
// consume numbers.
//
// Transaction: read the bill doc; if it already has a billNumber
// (reprint) return it unchanged — reprints must keep the same invoice
// number. Otherwise increment restaurants/{rid}/counters/billing
// .nextBill and stamp the number onto the bill doc atomically.
//
// Returns the number, or null when there's no bill doc to stamp
// (e.g. takeaway orders printed without a tableBill) — the bill
// renders without a Bill No. line in that case.
export async function ensureBillNumber(restaurantId, billId, opts = {}) {
  if (!billId) return null;
  const fs = opts.db || db;
  const billRef = doc(fs, 'restaurants', restaurantId, 'tableBills', billId);
  const counterRef = doc(fs, 'restaurants', restaurantId, 'counters', 'billing');
  try {
    return await runTransaction(fs, async (txn) => {
      const billSnap = await txn.get(billRef);
      if (!billSnap.exists()) return null;
      const existing = billSnap.data().billNumber;
      if (typeof existing === 'number' && existing > 0) return existing;

      const counterSnap = await txn.get(counterRef);
      const next = (counterSnap.exists() ? (Number(counterSnap.data().nextBill) || 0) : 0) + 1;
      txn.set(counterRef, { nextBill: next, updatedAt: serverTimestamp() }, { merge: true });
      txn.update(billRef, withActor({ billNumber: next }));
      return next;
    });
  } catch (err) {
    // Numbering must never block the actual print — worst case the
    // bill goes out without a Bill No. (same graceful stance as the
    // orderNumber fallback in createOrder).
    console.warn('[ensureBillNumber] failed:', err?.message);
    return null;
  }
}
...
export async function linkOrderToBill(restaurantId, orderId, billId, opts = {}) {
  if (!orderId || !billId) return;
  const fs = opts.db || db;
  return updateDoc(
    doc(fs, 'restaurants', restaurantId, 'orders', orderId),
    withActor({ billId })
  );
}
...
export async function freeTableSession(restaurantId, tableCode, billId, opts = {}) {
  const fs = opts.db || db;
  const tasks = [];
  ...
  if (code) tasks.push(
    setDoc(
      doc(fs, 'restaurants', restaurantId, 'tableSessions', code),
...
export async function markTableSeated(restaurantId, tableCode, info = {}, opts = {}) {
  const fs = opts.db || db;
  const code = String(tableCode || '').trim();
  if (!code) return;
  return setDoc(
...
export async function saveDayClose(restaurantId, dateKey, data, opts = {}) {
  const fs = opts.db || db;
  return setDoc(
    doc(fs, 'restaurants', restaurantId, 'dayCloses', dateKey),
    withActor({ ...data, dateKey, locked: true, closedAt: serverTimestamp() }),
    { merge: true }
  );
}

export async function getDayClose(restaurantId, dateKey, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDoc(doc(fs, 'restaurants', restaurantId, 'dayCloses', dateKey));
  return snap.exists() ? { id: snap.id, ...snap.data() } : null;
}

export async function getDayCloses(restaurantId, max = 30, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(
    query(collection(fs, 'restaurants', restaurantId, 'dayCloses'), orderBy('dateKey', 'desc'), limit(max))
  );
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

export async function reopenDay(restaurantId, dateKey, opts = {}) {
  const fs = opts.db || db;
  return deleteDoc(doc(fs, 'restaurants', restaurantId, 'dayCloses', dateKey));
}
...
export async function resolveWaiterCall(restaurantId, callId) {
  ...
  logStaffActivity(db, restaurantId, { action: 'call_resolved', refType: 'call', refId: callId });
  return res;
}
...
export async function createMenuItem(restaurantId, data, opts = {}) {
  const fs = opts.db || db;
  return addDoc(
    collection(fs, 'restaurants', restaurantId, 'menuItems'),
    withActor({
...
export async function getCombos(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(
    query(collection(fs, 'restaurants', restaurantId, 'combos'), orderBy('createdAt', 'desc'))
  );
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

export async function createCombo(restaurantId, data, opts = {}) {
  const fs = opts.db || db;
  return addDoc(
    collection(fs, 'restaurants', restaurantId, 'combos'),
    withActor({ ...data, isActive: true, createdAt: serverTimestamp() })
  );
}

export async function updateCombo(restaurantId, comboId, data, opts = {}) {
  const fs = opts.db || db;
  return updateDoc(doc(fs, 'restaurants', restaurantId, 'combos', comboId), withActor(data));
}

export async function deleteCombo(restaurantId, comboId, opts = {}) {
  const fs = opts.db || db;
  return deleteDoc(doc(fs, 'restaurants', restaurantId, 'combos', comboId));
}
...
export async function getTodayAnalytics(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const today = todayKey();
  const snap = await getDoc(doc(fs, 'restaurants', restaurantId, 'analytics', today));
  if (!snap.exists()) return { date: today, totalVisits: 0, uniqueVisitors: 0, repeatVisitors: 0 };
  return snap.data();
}

export async function getWaiterCallsCount(restaurantId, days = 30, opts = {}) {
  const fs = opts.db || db;
  const since = new Date();
  since.setDate(since.getDate() - days);
  const snap = await getDocs(collection(fs, 'restaurants', restaurantId, 'waiterCalls'));
...
export async function createOrder(restaurantId, data, opts = {}) {
  // opts.db lets an RBAC staff session create through staffDb (their token
  // rides along). Defaults to the admin db so existing callers are unchanged.
  const fs = opts.db || db;
  const ordersCol = collection(fs, 'restaurants', restaurantId, 'orders');
  const orderRef = doc(ordersCol); // pre-allocate a doc ID so we can write inside the txn
  const dayKey = todayKeyLocal();
  const counterRef = doc(fs, 'restaurants', restaurantId, 'orderCounters', dayKey);
  ...
  try {
    ...
    // Staff accountability trail — no-op for anonymous customer orders.
    logStaffActivity(fs, restaurantId, {
      action: 'order_placed', refType: 'order', refId: orderRef.id,
      tableNumber: data.tableNumber, amount: data.total,
      detail: `${(data.items || []).reduce((s, it) => s + (Number(it.qty) || 1), 0)} items`,
    });
    return orderRef.id;
  } catch (err) {
    ...
    // Audit #15: the fallback protects the customer flow but used to be
    // invisible to the owner — recurring counter failures (rules drift,
    // clock skew) produced number-less orders nobody noticed until the
    // kitchen complained. Report to Sentry (no-op without DSN). Dynamic
    // import keeps @sentry/nextjs out of this module's customer-page
    // dependency graph until the moment it's actually needed.
    import('@sentry/nextjs')
      .then(S => S.captureMessage('createOrder: counter transaction failed — order created without orderNumber', {
        level: 'warning',
        tags: { area: 'order-numbering' },
        extra: { restaurantId, code },
      }))
      .catch(() => {});
...
export async function attachOrderToBill(restaurantId, billId, orderId, opts = {}) {
...
export async function updatePaymentStatus(restaurantId, orderId, paymentStatus, opts = {}) {
  const fs = opts.db || db;
  return updateDoc(
    doc(fs, 'restaurants', restaurantId, 'orders', orderId),
...
export async function cancelOrder(restaurantId, orderId, reason = '', opts = {}) {
  const fs = opts.db || db;
  return updateDoc(
    doc(fs, 'restaurants', restaurantId, 'orders', orderId),
    withActor({
...
export async function updateOrderStatus(restaurantId, orderId, status, opts = {}) {
  const fs = opts.db || db;
  const res = await updateDoc(
    doc(fs, 'restaurants', restaurantId, 'orders', orderId),
    withActor({ status, updatedAt: serverTimestamp() })
  );
  // Accountability: cancellations are THE classic fraud-watch action;
  // served closes the loop. Routine pending→preparing flips stay quiet.
  if (status === 'cancelled' || status === 'served') {
    logStaffActivity(fs, restaurantId, {
      action: status === 'cancelled' ? 'order_cancelled' : 'order_served',
...
export async function getStaffMembers(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(collection(fs, 'restaurants', restaurantId, 'staff'));
  return snap.docs.map(d => ({ id: d.id, ...d.data() }))
...
// verifyStaffLogin was removed (2026-06-11 security audit): it compared
// plaintext PINs client-side and was dead code — every caller goes
// through /api/staff/login, which verifies bcrypt hashes server-side
// with rate limiting and lazy migration of legacy plaintext PINs.

// ─── Table Sessions ────────────────────────────────────────────
// Each table has one session doc. Token validated on menu load & order.
// Default expiry: 3 hours. Admin can activate/clear anytime.

// Cryptographically strong session id. The sid is the ONLY thing
// authenticating an anonymous dine-in session (QR → bill → order), so
// Math.random() — predictable, seedable — was not good enough: a
// motivated attacker could derive upcoming sids and forge orders onto
// other tables' bills. Web Crypto is available in every browser we
// support AND in Node 18+ (SSR path), with a defensive fallback.
function secureSid() {
  const bytes = new Uint8Array(16);
  if (typeof globalThis.crypto?.getRandomValues === 'function') {
    globalThis.crypto.getRandomValues(bytes);
  } else {
    // Should never happen on our supported platforms; better a weak
    // sid than a crashed activation flow on some ancient webview.
    for (let i = 0; i < bytes.length; i++) bytes[i] = Math.floor(Math.random() * 256);
  }
  return Array.from(bytes, b => b.toString(36).padStart(2, '0')).join('').slice(0, 20);
}

export async function activateTableSession(restaurantId, tableNumber, hoursValid = 3, opts = {}) {
  const fs = opts.db || db;
  const expiresAt = new Date(Date.now() + hoursValid * 60 * 60 * 1000);
  // sid: unguessable random ID embedded in the QR URL. By default a fresh one
  // is minted on every activation. Pass opts.sid to REUSE the table's existing
  // token — the floor's seat→activate does this so a pre-printed QR sticker
  // keeps working across parties (only isActive + expiry flip, the sid stays).
  const sid = opts.sid || secureSid();
  const ref = doc(fs, 'restaurants', restaurantId, 'tableSessions', String(tableNumber));
  await setDoc(ref, {
...
export async function clearTableSession(restaurantId, tableNumber, opts = {}) {
  const fs = opts.db || db;
  const ref = doc(fs, 'restaurants', restaurantId, 'tableSessions', String(tableNumber));
...
export async function getCoupons(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(collection(fs, 'restaurants', restaurantId, 'coupons'));
  return snap.docs
    .map(d => ({ id: d.id, ...d.data() }))
    .sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
}

export async function createCoupon(restaurantId, data, opts = {}) {
  const fs = opts.db || db;
  return addDoc(collection(fs, 'restaurants', restaurantId, 'coupons'), withActor({
    ...data,
...
export async function updateCoupon(restaurantId, couponId, data, opts = {}) {
  const fs = opts.db || db;
  return updateDoc(doc(fs, 'restaurants', restaurantId, 'coupons', couponId), withActor(data));
}

export async function deleteCoupon(restaurantId, couponId, opts = {}) {
  const fs = opts.db || db;
  return deleteDoc(doc(fs, 'restaurants', restaurantId, 'coupons', couponId));
}
...
export async function getFeedback(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(
    query(
      collection(fs, 'restaurants', restaurantId, 'feedback'),
      orderBy('createdAt', 'desc'),
      limit(200)
    )
  );
...
export async function updateOrderStatusAs(firestore, restaurantId, orderId, status) {
  ...
  if (status === 'cancelled' || status === 'served') {
    logStaffActivity(firestore, restaurantId, {
      action: status === 'cancelled' ? 'order_cancelled' : 'order_served',
      refType: 'order', refId: orderId,
    });
  }
...
export async function resolveWaiterCallAs(firestore, restaurantId, callId) {
  ...
  logStaffActivity(firestore, restaurantId, { action: 'call_resolved', refType: 'call', refId: callId });
  return res;
}
...
async function markOrderItemTimestampAs(firestore, restaurantId, orderId, itemIdx, field) {
  ...
  // Captured inside the txn for the activity entry written after commit
  // (logging inside the txn would duplicate on retries).
  let logged = null;
  await runTransaction(firestore, async (txn) => {
    logged = null; // reset on txn retry
    ...
    logged = { name: items[itemIdx]?.name || 'item', table: data.tableNumber };
  ...
  if (logged) {
    logStaffActivity(firestore, restaurantId, {
      action: field === 'readyAt' ? 'item_ready' : 'item_served',
      refType: 'order', refId: orderId,
      tableNumber: logged.table, detail: logged.name,
    });
  }
}
...
export async function markOrderPaid(restaurantId, orderId, paymentStatus, extras = {}) {
  ...
  if (_PAID_STATUSES.includes(paymentStatus)) {
    logStaffActivity(db, restaurantId, {
      action: 'payment_marked', refType: 'order', refId: orderId, detail: paymentStatus,
    });
...
export async function getCustomers(restaurantId, max = 2000, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(query(
    collection(fs, 'restaurants', restaurantId, 'customers'),
...
export async function createCustomer(restaurantId, data, opts = {}) {
  const fs = opts.db || db;
  const phone = normalizePhone(data.phone);
  if (!phone) throw new Error('A valid 10-digit phone is required');
  const nowISO = new Date().toISOString();
  await setDoc(doc(fs, 'restaurants', restaurantId, 'customers', phone), withActor({
...
export async function updateCustomer(restaurantId, phone, data, opts = {}) {
  const fs = opts.db || db;
  ...
  return updateDoc(doc(fs, 'restaurants', restaurantId, 'customers', phone), withActor(out));
}

// Adjust loyalty points by a signed delta (bonus + / redemption −).
// Uses increment() so concurrent taps are safe.
export async function adjustCustomerPoints(restaurantId, phone, delta, opts = {}) {
  const fs = opts.db || db;
  const d = Math.round(Number(delta) || 0);
  if (!d) return;
  return updateDoc(
    doc(fs, 'restaurants', restaurantId, 'customers', phone),
    withActor({ pointsAdjust: increment(d), pointsAdjustedAt: serverTimestamp() })
  );
}

export async function deleteCustomer(restaurantId, phone, opts = {}) {
  const fs = opts.db || db;
  return deleteDoc(doc(fs, 'restaurants', restaurantId, 'customers', phone));
}
...
export async function createCampaign(restaurantId, data, opts = {}) {
  const fs = opts.db || db;
  return addDoc(collection(fs, 'restaurants', restaurantId, 'campaigns'), withActor({
...
export async function deleteCampaign(restaurantId, campaignId, opts = {}) {
  const fs = opts.db || db;
  return deleteDoc(doc(fs, 'restaurants', restaurantId, 'campaigns', campaignId));
}
...
export async function createWaitlistEntry(restaurantId, data, opts = {}) {
  const fs = opts.db || db;
  return addDoc(collection(fs, 'restaurants', restaurantId, 'waitlist'), withActor({
...
export async function updateWaitlistEntry(restaurantId, entryId, data, opts = {}) {
  const fs = opts.db || db;
  return updateDoc(
    doc(fs, 'restaurants', restaurantId, 'waitlist', entryId),
...
export async function setWaitlistStatus(restaurantId, entryId, status, extra = {}) {
  ...
  return updateDoc(doc(fs, 'restaurants', restaurantId, 'waitlist', entryId), withActor(patch));
}

export async function deleteWaitlistEntry(restaurantId, entryId, opts = {}) {
  const fs = opts.db || db;
  return deleteDoc(doc(fs, 'restaurants', restaurantId, 'waitlist', entryId));
}
...
export async function getStaffRoles(restaurantId, opts = {}) {
  const fs = opts.db || db;
  const snap = await getDocs(query(
    collection(fs, 'restaurants', restaurantId, 'staffRoles'),
...
export async function setStaffRole(restaurantId, staffId, roleId, opts = {}) {
  // Unified role model (26 May 2026): a staffer has exactly ONE role. When the
  // owner switches it on a staff card we update BOTH fields together:
  //   - built-in station role → role='kitchen'|'waiter', roleId=null
  //   - custom role           → role='staff',            roleId=<staffRoles id>
  // opts.role carries the base ('kitchen'|'waiter'|'staff'); callers that omit
  // it (legacy) just set roleId, leaving the base untouched.
  const patch = { roleId: roleId || null };
