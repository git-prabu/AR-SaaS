...
export async function markOrderPaid(restaurantId, orderId, paymentStatus, extras = {}) {
  ...
  if (_PAID_STATUSES.includes(paymentStatus)) {
    await _releaseAwaitingPaymentIfNeeded(db, restaurantId, orderId);
    await _autoCloseBillIfAllPaid(db, restaurantId, orderId);
  }
}
...
export async function markOrderPaidAs(firestore, restaurantId, orderId, paymentStatus, extras = {}) {
  ...
  await updateDoc(orderRef, payload);
  if (_PAID_STATUSES.includes(paymentStatus)) {
    await _releaseAwaitingPaymentIfNeeded(firestore, restaurantId, orderId);
    await _autoCloseBillIfAllPaid(firestore, restaurantId, orderId);
  }
}
