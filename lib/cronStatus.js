...
export async function withCronStatus(cronName, work, res) {
  ...
  try {
    ...
    // Sentry (2026-06-11 audit Phase B): a failed cron is exactly the
    // kind of silent breakage the owner can't see — backup stops,
    // summaries stop. captureException is a no-op when Sentry isn't
    // configured, so this is safe pre-setup.
    try {
      Sentry.captureException(err, { tags: { cron: cronName }, extra: { durationMs } });
    } catch {}
