...
export function buildBillHtml(orders, opts = {}) {
  ...
  // Bill reference = the order number(s) on this bill. A formal sequential
  // invoice number can layer on later via opts.billNumber.
  const nums = list.map(o => o.orderNumber).filter(n => typeof n === 'number' && n > 0);
  const billRef = opts.billNumber || (nums.length ? nums.map(n => '#' + n).join(', ') : '');
  // Payment method — shown only once the bill is actually paid.
  const PAY = { paid_cash: 'Cash', paid_card: 'Card', paid_online: 'UPI / Online', paid: 'Paid' };
  const payStatuses = [...new Set(list.map(o => o.paymentStatus).filter(Boolean))];
  const paymentLabel = payStatuses.filter(s => PAY[s]).length > 1 ? 'Multiple'
    : (PAY[payStatuses.find(s => PAY[s])] || '');
  ...
  // Money row that hides itself when zero. `neg` renders -X (discount);
  // `signed` prefixes + for a positive value (round-off); `always` forces it.
  const moneyRow = (label, val, o = {}) => {
    const n = Number(val) || 0;
    if (!o.always && n === 0) return '';
    const shown = o.neg ? `-${Math.abs(n).toFixed(2)}`
      : (o.signed && n > 0 ? `+${n.toFixed(2)}` : n.toFixed(2));
    return `<div class="trow"><span>${label}</span><span>${shown}</span></div>`;
  };
...
<style>
  @page { size: 80mm auto; margin: 0; }
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { width: 80mm; padding: 6mm 5mm 80px 5mm; font-family: 'Courier New', ui-monospace, monospace; color: #000; font-size: 12px; -webkit-print-color-adjust: exact; }
  @media print { body { padding-bottom: 0; } }
  .ctr { text-align: center; }
...
<body>
  ...
  ${billRef ? `<div class="meta"><span>Bill ${escapeHtml(billRef)}</span><span></span></div>` : ''}
  ${r.hsnCode ? `<div class="meta"><span>HSN/SAC ${escapeHtml(String(r.hsnCode))}</span><span></span></div>` : ''}
  <div class="hr"></div>
  ...
  <div class="hr"></div>
  ${moneyRow('Subtotal', subtotal, { always: true })}
  ${moneyRow('Service charge', serviceCharge)}
  ${moneyRow(`CGST ${(gstPct / 2).toFixed(1)}%`, cgst)}
  ${moneyRow(`SGST ${(gstPct / 2).toFixed(1)}%`, sgst)}
  ${moneyRow(couponCode ? `Discount (${escapeHtml(couponCode)})` : 'Discount', discount, { neg: true })}
  ${moneyRow('Round off', roundOff, { signed: true })}
  <div class="tot"><span>TOTAL</span><span>&#8377;${Math.round(total)}</span></div>
  ${paymentLabel ? `<div class="trow" style="margin-top:6px"><span>Payment</span><span>${escapeHtml(paymentLabel)}</span></div>` : ''}
  <div class="hr"></div>
