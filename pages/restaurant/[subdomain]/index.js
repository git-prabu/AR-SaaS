...
export default function RestaurantMenu({ restaurant: initialRestaurant, menuItems: initialItems, offers: initialOffers, combos: initialCombos, error }) {
  ...
  // QR fallback (desktop). When customer is on a non-mobile device that
  // can't open `upi://` deep links, we render a scannable QR code of the
  // same UPI URL — they scan with their phone's UPI app to pay.
  ...
  useEffect(() => {
    ...
    setIsDesktop(!(mobileUA || hasTouch));
  }, []);
              ...
              <div style={{ padding: '0 22px calc(env(safe-area-inset-bottom, 20px) + 24px)', overflowY: 'auto', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', flex: 1 }}>
                ...
                {billPaymentState === 'paid' ? (
                  ...
                  const runUpiPayment = async (app) => {
                    ...
                    // no gateway. On mobile we open the chosen app via
                    // its scheme. On desktop the upi:// scheme has nothing
                    ...
                    if (restaurant?.upiId) {
                      ...
                      if (isDesktop) {
                        ...
                        try {
                          ...
                          const dataUrl = await QRCode.toDataURL(upiUrl, {
                            width: 280, margin: 1,
                            color: { dark: '#1E1B18', light: '#FFFFFF' },
                          });
                      ...
                      // Mobile path — open the deep link, switch to the
                      // "I've paid" sub-step on return.
                      setUpiOpened(true);
                      window.open(upiUrl, '_self');
                      return;
                    }
                  ...
                  // Higher-fidelity inline SVG brand marks. Each is the
                  // brand's actual primary colour with the closest possible
                  // monogram approximation given the constraint of pure
                  // inline SVG (no external images, no fonts beyond what
                  // the page already loads). Drawn at 40x40 with rounded
                  // 9px corners to match Material/iOS app-icon convention.
                  //
                  // Google Pay — canonical 4-color "G" path (the real
                  // Google logo, simplified path data).
                  // PhonePe — purple square + a stylized white "Pe" with
                  // ₹ inside the loop (PhonePe's real mark).
                  // Paytm — light-blue gradient square + white "paytm"
                  // wordmark with the ₹ tucked under the "y".
                  // Amazon Pay — black square + "amazon" wordmark + the
                  // signature orange smile arrow + "pay" subtext.
                  const BRAND_LOGO = {
                    gpay: (
                      <svg width="40" height="40" viewBox="0 0 40 40" aria-hidden="true">
                        <rect width="40" height="40" rx="9" fill="#FFFFFF" stroke="rgba(0,0,0,0.08)" />
                    ...
                    paytm: (
                      <svg width="40" height="40" viewBox="0 0 40 40" aria-hidden="true">
                        <defs>
                          <linearGradient id="pp_paytm" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0" stopColor="#00B9F1" />
                            <stop offset="1" stopColor="#00387B" />
                          </linearGradient>
                        </defs>
                        <rect width="40" height="40" rx="9" fill="url(#pp_paytm)" />
                        <text x="20" y="22" textAnchor="middle" fontFamily="Inter,Arial,sans-serif" fontSize="10" fontWeight="900" fill="#FFFFFF" letterSpacing="-0.3">paytm</text>
                    ...
                    cash: (
                      <svg width="32" height="32" viewBox="0 0 32 32" aria-hidden="true">
                        ...
                        <rect x="6" y="10" width="20" height="12" rx="1.5" stroke="#2D8B4E" strokeWidth="1.6" fill="none" />
                        <circle cx="16" cy="16" r="3" stroke="#2D8B4E" strokeWidth="1.4" fill="none" />
                        <text x="16" y="18" textAnchor="middle" fontFamily="Inter,Arial,sans-serif" fontSize="6" fontWeight="900" fill="#2D8B4E">₹</text>
                      </svg>
                    ),
                    card: (
                      <svg width="32" height="32" viewBox="0 0 32 32" aria-hidden="true">
                        <rect width="32" height="32" rx="8" fill="rgba(74,128,192,0.12)" />
                        <rect x="6" y="10" width="20" height="13" rx="2" stroke="#4A80C0" strokeWidth="1.6" fill="none" />
                        <line x1="6" y1="14" x2="26" y2="14" stroke="#4A80C0" strokeWidth="1.6" />
                        <line x1="9" y1="19" x2="14" y2="19" stroke="#4A80C0" strokeWidth="1.4" strokeLinecap="round" />
                      </svg>
                    ),
                  };
                  ...
                  // ── QR sub-step (desktop UPI fallback) ──
                  // Replaces the picker entirely while it's open. Customer
                  // scans with their phone's UPI app, then taps "I've paid"
                  // here to surface the request to the cashier.
                  if (qrPayload && qrFor) {
                    return (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        <div style={{
                          textAlign: 'center', padding: '20px 16px', borderRadius: 16,
                          background: darkMode ? 'rgba(255,245,232,0.04)' : '#FFFFFF',
                          border: `1px solid ${darkMode ? 'rgba(255,245,232,0.10)' : 'rgba(30,27,24,0.08)'}`,
                        }}>
                          <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.10em', textTransform: 'uppercase', color: darkMode ? 'rgba(255,245,232,0.55)' : 'rgba(30,27,24,0.55)', marginBottom: 10 }}>
                            Scan with any UPI app
                          </div>
                          <div style={{ display: 'inline-block', padding: 10, background: '#FFFFFF', borderRadius: 12, boxShadow: '0 2px 14px rgba(0,0,0,0.06)' }}>
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={qrPayload} alt="UPI payment QR" width="240" height="240" style={{ display: 'block' }} />
                          </div>
                          <div style={{ marginTop: 14, fontSize: 22, fontWeight: 800, color: darkMode ? '#FFF5E8' : '#1E1B18', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '-0.02em' }}>
                            ₹{qrFor.amount}
                          </div>
                          <div style={{ marginTop: 4, fontSize: 13, fontFamily: "'JetBrains Mono', monospace", color: darkMode ? 'rgba(255,245,232,0.55)' : 'rgba(30,27,24,0.55)', letterSpacing: '0.02em' }}>
                            {qrFor.vpa}
                          </div>
                          <div style={{ marginTop: 10, fontSize: 12, color: darkMode ? 'rgba(255,245,232,0.40)' : 'rgba(30,27,24,0.45)', lineHeight: 1.5 }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            border: 'none', background: 'transparent',
                            color: darkMode ? 'rgba(255,245,232,0.45)' : 'rgba(30,27,24,0.45)',
                            fontSize: 12, fontWeight: 600, fontFamily: 'Inter,sans-serif', cursor: 'pointer',
                          }}>
                          ← Back to payment options
                        </button>
