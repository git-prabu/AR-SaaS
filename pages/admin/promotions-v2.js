...
const STATUS_META = {
  ...
  inactive:  { label: 'INACTIVE',  bg: 'var(--card-3)',           color: A.mutedText },
...
const inputStyle = {
  ...
  background: A.shellDarker, border: '1px solid var(--line)', borderRadius: 9,
...
export default function PromotionsV2() {
  ...
  if (!ready) return (<div className="ok-root" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center' }}><PageHead title="Promotions" /><div style={{ color: 'var(--tx-3)', fontFamily: 'var(--font-body)', fontSize: 14 }}>Loading…</div></div>);
  if (!canView) return (<div className="ok-root" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24 }}><PageHead title="Promotions" /><div style={{ textAlign: 'center', maxWidth: 360 }}><div style={{ fontSize: 44, marginBottom: 12 }}>🔒</div><div style={{ color: 'var(--tx)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, marginBottom: 6 }}>No access</div><div style={{ color: 'var(--tx-3)', fontFamily: 'var(--font-body)', fontSize: 13.5, lineHeight: 1.6 }}>Your role doesn’t include Promotions. Ask the owner to grant it.</div></div></div>);
  if (planAllowsFeature === false) return (<div className="ok-root" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24 }}><PageHead title="Promotions" /><div style={{ textAlign: 'center', maxWidth: 380 }}><div style={{ fontSize: 44, marginBottom: 12 }}>✨</div><div style={{ color: 'var(--tx)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, marginBottom: 6 }}>Upgrade to use Promotions</div><div style={{ color: 'var(--tx-3)', fontFamily: 'var(--font-body)', fontSize: 13.5, lineHeight: 1.6, marginBottom: 16 }}>Offers, coupons &amp; combos are available on a higher plan.</div><Link href="/admin/subscription-v2" style={{ display: 'inline-block', padding: '10px 20px', borderRadius: 10, background: 'var(--accent)', color: 'var(--accent-ink)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>View plans →</Link></div></div>);

  return (
    <OkShell
      active="menu"
      eyebrow="Catalog · offers, coupons & combos"
      title="Promotions"
      brand="Promotions"
      headRight={
        <button onClick={() => openCreate(activeTabMeta.singular)} style={{ padding: '9px 18px', borderRadius: 10, border: 'none', background: A.accent, color: A.cream, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: A.font }}>+ New {activeTabMeta.singular}</button>
      }
    >
      ...
      <div style={{ fontFamily: A.font }}>
        <style>{`
          ...
          .promo-tab:hover:not(.active) { background: var(--card-3); color: var(--tx); }
          .promo-row:hover { box-shadow: 0 4px 18px rgba(0,0,0,0.35); }
          .promo-input:focus { border-color: var(--gold) !important; box-shadow: 0 0 0 3px rgba(196,168,109,0.15); }
          .promo-input::placeholder { color: var(--tx-3); }
          .promo-item-card:hover { background: var(--card-3); }
        `}</style>
...
PromotionsV2.getLayout = (page) => page;
...
function ComboRow({ c, menuItems, idx, onEdit, onToggle, onDelete, busy }) {
  ...
  const stripe = visible ? A.warning : 'var(--line)';
    ...
    <>
      {form.title && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{
            background: 'var(--card-2)', borderRadius: 12, padding: '14px 18px',
...
function Toggle({ on, onClick, busy }) {
  return (
    <div onClick={busy ? undefined : onClick} style={{
      ...
      background: on ? A.success : 'var(--card-3)',
      position: 'relative', transition: 'background 0.2s',
      opacity: busy ? 0.6 : 1,
    }}>
      <div style={{
        width: 18, height: 18, borderRadius: '50%', background: 'var(--tx)',
  ...
  return (
    <div style={{
      ...
      <button type="button" onClick={onSave} disabled={saving} style={{
        ...
        background: A.accent, color: A.cream,
