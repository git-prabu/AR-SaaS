...
export default function AdminPurchaseOrders() {
  ...
  // Print an A4 purchase order (buyer = this restaurant, supplier = vendor)
  // the owner can hand or email to a vendor. Opens a print window from the
  // PO data already on the page (no re-fetch).
  const printPO = (p) => {
    const esc = (s) => String(s == null ? '' : s).replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
    const restName = userData?.restaurantName || staffSession?.restaurantName || 'Restaurant';
    const items = Array.isArray(p.items) ? p.items : [];
    const subtotal = items.reduce((s, it) => s + (Number(it.qty) || 0) * (Number(it.rate) || 0), 0);
    const taxPct = Number(p.taxPercent) || 0;
    const taxAmt = subtotal * taxPct / 100;
    const total = Number(p.total) || (subtotal + taxAmt);
    const poNo = '#' + String(p.id || '').slice(-6).toUpperCase();
    const itemRows = items.map((it, i) => {
      const amt = (Number(it.qty) || 0) * (Number(it.rate) || 0);
      return `<tr><td>${i + 1}</td><td>${esc(it.name)}</td><td class="r">${esc(it.qty)}${it.unit && it.unit !== '—' ? ' ' + esc(it.unit) : ''}</td><td class="r">${(Number(it.rate) || 0).toFixed(2)}</td><td class="r">${amt.toFixed(2)}</td></tr>`;
    }).join('');
    const win = window.open('', '_blank', 'width=820,height=920');
    if (!win) { toast.error('Allow pop-ups to print the PO'); return; }
    win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>PO ${esc(poNo)}</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  body{font-family:Inter,Arial,sans-serif;color:#1A1A1A;padding:32px;font-size:13px}
  h1{font-size:22px;letter-spacing:-0.4px}
  .top{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #1A1A1A;padding-bottom:14px;margin-bottom:18px}
  .muted{color:#666;font-size:12px}
  .label{font-size:10px;text-transform:uppercase;letter-spacing:0.08em;color:#999;margin-bottom:3px}
  .meta{display:flex;gap:40px;margin-bottom:20px}
  table{width:100%;border-collapse:collapse;margin-bottom:16px}
  th{text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:0.06em;color:#999;border-bottom:1px solid #ddd;padding:8px 6px}
  td{padding:8px 6px;border-bottom:1px solid #f0f0f0}
  .r{text-align:right}
  .totals{margin-left:auto;width:260px}
  .totals .row{display:flex;justify-content:space-between;padding:4px 0}
  .totals .grand{border-top:2px solid #1A1A1A;margin-top:6px;padding-top:8px;font-weight:700;font-size:16px}
  .note{margin-top:18px;font-size:12px;color:#444}
  .ft{margin-top:36px;font-size:11px;color:#999;text-align:center}
  @media print{@page{margin:14mm}}
</style></head><body>
  <div class="top">
    <div><h1>${esc(restName)}</h1><div class="muted">Purchase Order</div></div>
    <div style="text-align:right"><div class="label">PO No.</div><div style="font-weight:700">${esc(poNo)}</div><div class="muted" style="margin-top:6px">${esc(p.date || '')}</div></div>
  </div>
  <div class="meta">
    <div><div class="label">Supplier</div><div style="font-weight:700">${esc(p.vendorName || 'Unassigned vendor')}</div></div>
    <div><div class="label">Status</div><div style="font-weight:700;text-transform:capitalize">${esc(p.status || 'draft')}</div></div>
  </div>
  <table>
    <thead><tr><th>#</th><th>Item</th><th class="r">Qty</th><th class="r">Rate (Rs.)</th><th class="r">Amount (Rs.)</th></tr></thead>
    <tbody>${itemRows || '<tr><td colspan="5" class="muted">No items</td></tr>'}</tbody>
  </table>
  <div class="totals">
    <div class="row"><span>Subtotal</span><span>Rs.${subtotal.toFixed(2)}</span></div>
    ${taxPct > 0 ? `<div class="row"><span>Tax (${taxPct}%)</span><span>Rs.${taxAmt.toFixed(2)}</span></div>` : ''}
    <div class="row grand"><span>Total</span><span>Rs.${Math.round(total)}</span></div>
  </div>
  ${p.note ? `<div class="note"><strong>Note:</strong> ${esc(p.note)}</div>` : ''}
  <div class="ft">Generated by HaloHelm</div>
</body></html>`);
    win.document.close();
    setTimeout(() => { try { win.print(); } catch { /* user closed it */ } }, 400);
  };
              ...
              return (
                <div key={p.id} style={{ background: A.shell, border: A.border, borderRadius: 13, padding: '15px 17px', boxShadow: A.cardShadow }}>
                  ...
                  <div style={{ marginTop: 13, display: 'flex', gap: 7, flexWrap: 'wrap', alignItems: 'center' }}>
                    <ActBtn onClick={() => printPO(p)} busy={busy} color={A.ink}>Print</ActBtn>
