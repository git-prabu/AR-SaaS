        ...
        <div style={S.page}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              ...
              <button onClick={() => { if (atLimit) { toast.error(`Item limit reached (${itemsUsed}/${maxItems}). Upgrade your plan.`); return; } setShowForm(!showForm); }} style={{ ...S.btn, background: showForm ? '#F2F0EC' : '#1E1B18', color: showForm ? '#1E1B18' : '#FFF5E8', border: showForm ? '1.5px solid rgba(42,31,16,0.12)' : 'none' }}>
          ...
          {/* Plan usage pill */}
          {restaurant && (
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, background: atLimit ? 'rgba(224,90,58,0.08)' : 'rgba(90,138,106,0.08)', border: `1px solid ${atLimit ? 'rgba(224,90,58,0.25)' : 'rgba(90,138,106,0.25)'}`, borderRadius:30, padding:'6px 14px', marginBottom:20 }}>
              <span style={{ fontSize:12, fontWeight:600, color: atLimit ? '#C04020' : '#3A7A50' }}>
                {atLimit ? '⚠️' : '✓'} {itemsUsed} / {maxItems} items used
              </span>
              {!atLimit && <span style={{ fontSize:11, color:'rgba(42,31,16,0.4)' }}>· {remaining} slot{remaining!==1?'s':''} remaining</span>}
          ...
          {bulkUploading && (
            <div style={{ background:'#fff', borderRadius:14, padding:'16px 20px', marginBottom:16, border:'1px solid rgba(90,138,106,0.2)' }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
                <span style={{ fontSize:13, fontWeight:600, color:'#1E1B18' }}>Publishing items…</span>
              ...
              {bulkProgress.current && <div style={{ fontSize:11, color:'rgba(42,31,16,0.45)', marginTop:6 }}>Processing: {bulkProgress.current}</div>}
            </div>
          )}
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
                  ...
                  <div style={{ position:'relative' }}>
                    ...
                    <div onClick={() => setShowCatDrop(d => !d)} style={{ ...S.input, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'space-between', userSelect:'none' }}>
                      <span style={{ color: form.category ? '#1E1B18' : 'rgba(42,31,16,0.3)' }}>{form.category || 'Select or type new…'}</span>
                    ...
                    {showCatDrop && (
                      <div style={{ position:'absolute', top:'100%', left:0, right:0, zIndex:20, background:'#fff', border:'1.5px solid rgba(42,31,16,0.1)', borderRadius:12, boxShadow:'0 8px 24px rgba(42,31,16,0.12)', marginTop:4, overflow:'hidden' }}>
                        <div style={{ padding:'10px 12px', borderBottom:'1px solid rgba(42,31,16,0.07)', display:'flex', gap:8 }}>
                          <input autoFocus style={{ flex:1, padding:'7px 10px', fontSize:13, border:'1.5px solid rgba(42,31,16,0.12)', borderRadius:8, outline:'none', fontFamily:'Inter,sans-serif', color:'#1E1B18' }} placeholder="Type new category…" value={newCatInput} onChange={e => setNewCatInput(e.target.value)}
                          ...
                          />
                          <button type="button" onClick={() => { if (!newCatInput.trim()) return; const cat = newCatInput.trim(); setForm(f => ({...f, category: cat})); if (!categories.includes(cat)) setCategories(c => [...c, cat].sort()); setNewCatInput(''); setShowCatDrop(false); }} style={{ padding:'7px 14px', borderRadius:8, border:'none', background:'#1E1B18', color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'Inter,sans-serif', flexShrink:0 }}>Add</button>
                        </div>
                        <div style={{ maxHeight:180, overflowY:'auto' }}>
                          {categories.length === 0 && <div style={{ padding:'12px 14px', fontSize:12, color:'rgba(42,31,16,0.4)' }}>No categories yet — type above to add one</div>}
                          {categories.map(cat => (
                            <div key={cat} onClick={() => { setForm(f => ({...f, category: cat})); setShowCatDrop(false); setNewCatInput(''); }} style={{ padding:'10px 14px', fontSize:13, color:'#1E1B18', cursor:'pointer', background: form.category === cat ? '#FFF5F0' : 'transparent', fontWeight: form.category === cat ? 600 : 400 }}
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
                  <div>
                    ...
                    <div style={{ position:'relative' }}>
                      <span style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)', fontSize:14, fontWeight:700, color:'rgba(42,31,16,0.5)' }}>₹</span>
                ...
                <div style={{ marginBottom:16 }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6 }}>
                    ...
                    <button type="button" className="calc-btn" onClick={handleAutoCalc} disabled={calcLoading} style={{ padding:'5px 14px', borderRadius:8, fontSize:12, fontWeight:600, fontFamily:'Inter,sans-serif', border:'1.5px solid rgba(224,90,58,0.4)', background:'#FFF5F2', color:'#E05A3A', cursor: calcLoading ? 'not-allowed' : 'pointer', opacity: calcLoading ? 0.7 : 1, transition:'all 0.15s', display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
                      {calcLoading ? <><span style={{ width:11, height:11, border:'2px solid #E05A3A', borderTopColor:'transparent', borderRadius:'50%', display:'inline-block', animation:'spin 0.7s linear infinite' }}/> Calculating…</> : '✦ Auto Calculate'}
                    </button>
                  </div>
                  ...
                  {calcLoading && calcDetail && <div style={{ fontSize:11, color:'rgba(224,90,58,0.7)', marginTop:5, fontStyle:'italic' }}>{calcDetail}</div>}
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
                    {['calories','protein','carbs','fats'].map(n => (
                      <div key={n}>
                        ...
                        <input className="inp" style={{ ...S.input, background: form[n] ? '#F0FBF5' : '#F7F5F2', borderColor: form[n] ? 'rgba(90,154,120,0.4)' : undefined }} type="number" min="0" value={form[n]} onChange={e=>setForm(f=>({...f,[n]:e.target.value}))} placeholder="0" />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <p style={{ textAlign:'center', fontSize:11, color:'rgba(42,31,16,0.4)', marginTop:10 }}>Item goes live immediately · AR unlocks after our team adds the 3D model</p>
              </form>
            </div>
          )}
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8 }}>
                      <div>
                        ...
                        <div style={{ fontSize:12, color:'rgba(42,31,16,0.45)', marginTop:2 }}>
                          {req.category && <span>{req.category}</span>}
                          {req.price  && <span style={{ marginLeft:8, fontWeight:600, color:'#E05A3A' }}>₹{req.price}</span>}
                        </div>
                      </div>
                      ...
                      {req.status === 'pending' && (
                        confirmCancel === req.id ? (
                          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                            <span style={{ fontSize:12, color:'rgba(42,31,16,0.6)' }}>Cancel AR request? Item stays on menu.</span>
...
function StatusBadge({ status }) {
  const map = {
    ...
    approved: { bg:'#8FC4A8', color:'#1A5A38', label:'AR Active' },
    rejected: { bg:'#F4A0B0', color:'#8B1A2A', label:'Rejected' },
  };
  const { bg, color, label } = map[status] || map.pending;
  return (
    <span style={{ padding:'4px 12px', borderRadius:30, fontSize:11, fontWeight:700, background:bg+'33', color, border:`1px solid ${bg}66`, flexShrink:0, whiteSpace:'nowrap' }}>
      {label}
    </span>
  );
}