...
export default function StaffV2() {
  ...
  if (!ready) return (<div className="ok-root" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center' }}><Head><title>Staff — HaloHelm</title></Head><div style={{ color: 'var(--tx-3)', fontFamily: 'var(--font-body)', fontSize: 14 }}>Loading…</div></div>);
  if (!canView) return (<div className="ok-root" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24 }}><Head><title>Staff — HaloHelm</title></Head><div style={{ textAlign: 'center', maxWidth: 360 }}><div style={{ fontSize: 44, marginBottom: 12 }}>🔒</div><div style={{ color: 'var(--tx)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, marginBottom: 6 }}>No access</div><div style={{ color: 'var(--tx-3)', fontFamily: 'var(--font-body)', fontSize: 13.5, lineHeight: 1.6 }}>Your role doesn’t include Staff management. Ask the owner to grant it.</div></div></div>);

  return (
    <OkShell
      active={null}
      eyebrow="People · staff logins"
      title="Staff"
      brand={restaurantName}
      headRight={
        <button onClick={openAdd} className="staff-add-btn" style={{ padding: '9px 18px', borderRadius: 10, border: 'none', background: A.accent, color: A.cream, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: A.font }}>+ Add Staff</button>
      }
    >
      ...
      <div style={{ fontFamily: A.font }}>
        <style>{`
          ...
          .staff-card:hover { box-shadow: 0 12px 32px rgba(0,0,0,0.45), 0 0 0 1px rgba(196,168,109,0.55); transform: translateY(-3px); }
          .staff-icon-btn:hover { background: var(--card-3); }
          .staff-filter-pill:hover:not(.active) { background: var(--card-3); color: var(--tx); }
          .staff-add-btn:hover { filter: brightness(1.08); }
          @media (max-width: 520px) { .staff-form-row-2 { grid-template-columns: 1fr !important; } }
        `}</style>

        {/* Restaurant code + filters + list */}
        <div>
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  background: A.accent, color: A.cream, fontSize: 12, fontWeight: 700,
                ...
                return (
                  <button key={f.key} className={`staff-filter-pill ${active ? 'active' : ''}`}
                    ...
                    style={{
                      ...
                      background: active ? A.accent : 'transparent', color: active ? A.cream : A.mutedText,
      ...
      {activityFor && (
        <div
          ...
          <div
            ...
            style={{
              width: 'min(1000px, 100%)', background: 'var(--surface)',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={handleSave} disabled={saving || !form.name || !form.username || (modal === 'add' && !form.pin)} style={{
                ...
                background: A.accent, color: A.cream, fontWeight: 700, fontSize: 13,
