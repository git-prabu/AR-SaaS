...
const A = {
  ...
  cream: 'var(--accent-ink)', ink: 'var(--tx)', accent: 'var(--accent)',
  shell: 'var(--card)', shellDarker: 'var(--card-2)',
  warning: 'var(--gold)', warningDim: 'var(--gold)',
  success: 'var(--success)', danger: 'var(--danger)',
  mutedText: 'var(--tx-2)', faintText: 'var(--tx-3)',
  subtleBg: 'var(--card-3)',
  border: '1px solid var(--line)',
  borderStrong: '1px solid rgba(255,255,255,0.16)',
  shadowCard: '0 1px 2px rgba(0,0,0,0.25)',
  forest: 'var(--card)', forestDarker: 'var(--card-2)',
  forestText: 'var(--tx)', forestTextMuted: 'var(--tx-2)', forestTextFaint: 'var(--tx-3)',
...
export default function ItemsV2() {
  ...
  if (!ready) return (<div className="ok-root" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center' }}><Head><title>Menu Items | HaloHelm</title></Head><div style={{ color: 'var(--tx-3)', fontFamily: 'var(--font-body)', fontSize: 14 }}>Loading…</div></div>);
  if (!canView) return (<div className="ok-root" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24 }}><Head><title>Menu Items | HaloHelm</title></Head><div style={{ textAlign: 'center', maxWidth: 360 }}><div style={{ fontSize: 44, marginBottom: 12 }}>🔒</div><div style={{ color: 'var(--tx)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, marginBottom: 6 }}>No access</div><div style={{ color: 'var(--tx-3)', fontFamily: 'var(--font-body)', fontSize: 13.5, lineHeight: 1.6 }}>Your role doesn’t include Menu Items. Ask the owner to grant it.</div></div></div>);

  return (
    <OkShell active="menu" eyebrow="Menu · items" title="Menu Items" brand="Menu">
                    ...
                    return (
                      <button key={cat} onClick={() => setCatFilter(cat)}
                        style={{
                          ...
                          background: active ? A.accent : A.shell,
                          color: active ? A.cream : A.mutedText,
...
const inputStyle = {
  ...
  fontSize: 13, color: 'var(--tx)', fontFamily: INTER,
  outline: 'none', boxSizing: 'border-box', transition: 'all 0.15s',
};
function Label({ children }) { return <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--tx-2)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6, fontFamily: INTER }}>{children}</label>; }
...
function IconBtn({ children, title, onClick, color = 'var(--tx-2)', disabled = false }) {
  return (
...
function FlagRow({ label, hint, on, onClick, color }) {
  return (
    <div onClick={onClick} style={{
      padding: '10px 14px', borderRadius: 10, background: 'var(--card-2)',
      border: `1px solid ${on ? color + '40' : 'var(--line)'}`,
      display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12,
      cursor: 'pointer', transition: 'all 0.15s',
    }}>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)', marginBottom: 1 }}>{label}</div>
        <div style={{ fontSize: 11, color: 'var(--tx-2)' }}>{hint}</div>
      </div>
      <div style={{
        width: 36, height: 20, borderRadius: 99,
        background: on ? color : 'var(--card-3)',
        position: 'relative', transition: 'background 0.2s', flexShrink: 0,
      }}>
        <div style={{
          width: 14, height: 14, borderRadius: '50%', background: 'var(--tx)',
...
function LoadingCard() {
  return (
    <div style={{ background: 'var(--card)', borderRadius: 14, border: '1px solid var(--line)', padding: '64px 32px', textAlign: 'center' }}>
      <div style={{ display: 'inline-block', width: 24, height: 24, border: '2px solid var(--card-3)', borderTopColor: '#C4A86D', borderRadius: '50%', animation: 'spin 0.8s linear infinite', marginBottom: 16 }} />
      <div style={{ fontSize: 13, color: 'var(--tx-2)' }}>Loading items…</div>
    </div>
  );
}
function EmptyCard({ titleText, subtitleText }) {
  return (
    <div style={{ background: 'var(--card)', borderRadius: 14, border: '1px solid var(--line)', padding: '64px 32px', textAlign: 'center' }}>
      ...
      <div style={{ fontWeight: 600, fontSize: 16, color: 'var(--tx)', marginBottom: 8, letterSpacing: '-0.2px' }}>{titleText}</div>
      <div style={{ fontSize: 13, color: 'var(--tx-2)', lineHeight: 1.6, maxWidth: 440, margin: '0 auto' }}>{subtitleText}</div>
    </div>
  );
}

ItemsV2.getLayout = (page) => page;