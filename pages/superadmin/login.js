      ...
      <div style={{minHeight:'100vh',background:G.bg,fontFamily:`'Bricolage Grotesque',Inter,sans-serif`,display:'flex',alignItems:'center',justifyContent:'center',padding:20,position:'relative',overflow:'hidden'}}>
        <style>{`
          ...
          *{box-sizing:border-box}
          .slci{width:100%;padding:12px 16px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.09);border-radius:8px;font-size:14px;color:rgba(255,255,255,0.82);outline:none;transition:all 0.2s;font-family:inherit;}
          .slci:focus{border-color:rgba(184,150,46,0.5);background:rgba(255,255,255,0.06);box-shadow:0 0 0 3px rgba(184,150,46,0.08);}
          .slci::placeholder{color:rgba(255,255,255,0.2);}
        `}</style>
        ...
        <div style={{width:'100%',maxWidth:380,position:'relative',zIndex:1}}>
          <div style={{textAlign:'center',marginBottom:36}}>
            ...
            <div style={{fontSize:12,color:G.gold,marginTop:8,letterSpacing:'0.1em',textTransform:'uppercase',fontFamily:`'DM Mono',monospace`,fontWeight:600}}>Super Admin</div>
          </div>
          ...
          <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:16,padding:32,boxShadow:'0 32px 64px rgba(0,0,0,0.5)'}}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <input className="slci" type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="admin@advertradical.com"/>
            </div>
            <div style={{marginBottom:28}}>
              ...
              <input className="slci" type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••"/>
            </div>
            <button onClick={handleSubmit} disabled={loading} style={{width:'100%',padding:'13px',borderRadius:8,border:`1px solid rgba(184,150,46,${loading?'0.15':'0.35'})`,background:loading?'transparent':'rgba(184,150,46,0.1)',color:loading?G.textDim:G.gold,fontFamily:'inherit',fontWeight:700,fontSize:14,cursor:loading?'not-allowed':'pointer',transition:'all 0.2s'}}>
