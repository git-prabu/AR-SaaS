...
/* ── Spice corner decoration (SVG) ── */
function SpiceDecor({ pos='tr', opacity=0.55 }) {
  const style = {
    position:'absolute', pointerEvents:'none', zIndex:0, opacity,
  ...
  return (
    <svg width="180" height="180" viewBox="0 0 180 180" style={style}>
      {pos==='tr' && <>
        {/* Cinnamon sticks */}
        <rect x="120" y="8" width="10" height="52" rx="5" fill="#C8895A" transform="rotate(32,125,34)"/>
        <rect x="134" y="6" width="10" height="52" rx="5" fill="#B87848" transform="rotate(28,139,32)"/>
        {/* Star anise */}
        <g transform="translate(80,20) rotate(15)">
          {[0,45,90,135,180,225,270,315].map(a=>(
            <ellipse key={a} cx="0" cy="-11" rx="3" ry="6" fill="#8B4513" opacity="0.8" transform={`rotate(${a})`}/>
          ))}
          <circle cx="0" cy="0" r="4" fill="#6B3410"/>
        </g>
        {/* Parsley */}
        <ellipse cx="148" cy="88" rx="14" ry="10" fill="#5A8A50" transform="rotate(-20,148,88)"/>
        <ellipse cx="162" cy="78" rx="12" ry="8" fill="#4A7A40" transform="rotate(10,162,78)"/>
        <ellipse cx="155" cy="70" rx="10" ry="7" fill="#6A9A60" transform="rotate(-5,155,70)"/>
        {/* Pepper */}
        <circle cx="100" cy="70" r="5" fill="#C04A28" opacity="0.7"/>
        <circle cx="90" cy="80" r="4" fill="#8B1A2A" opacity="0.6"/>
        <circle cx="110" cy="85" r="3.5" fill="#C04A28" opacity="0.5"/>
      </>}
      {pos==='tl' && <>
        {/* Herbs / leaves */}
        <ellipse cx="30" cy="40" rx="18" ry="11" fill="#4A8A50" transform="rotate(-30,30,40)"/>
        <ellipse cx="15" cy="28" rx="14" ry="8" fill="#5A9A60" transform="rotate(10,15,28)"/>
        <ellipse cx="48" cy="28" rx="12" ry="8" fill="#3A7A40" transform="rotate(-15,48,28)"/>
        {/* Cumin seeds */}
        <ellipse cx="70" cy="55" rx="5" ry="2.5" fill="#8B6020" transform="rotate(30,70,55)"/>
        <ellipse cx="60" cy="65" rx="5" ry="2.5" fill="#8B6020" transform="rotate(-20,60,65)"/>
        <ellipse cx="80" cy="68" rx="5" ry="2.5" fill="#8B6020" transform="rotate(50,80,68)"/>
        {/* Dots */}
        <circle cx="55" cy="15" r="4" fill="#F79B3D" opacity="0.6"/>
        <circle cx="20" cy="68" r="3" fill="#F79B3D" opacity="0.5"/>
      </>}
      {pos==='br' && <>
        {/* Chili */}
        <path d="M140,140 Q155,120 165,110 Q170,105 168,112 Q160,125 148,148Z" fill="#C04A28"/>
        <path d="M148,148 Q162,128 170,115" stroke="#8B1A2A" strokeWidth="2" fill="none"/>
        {/* Turmeric powder dots */}
        <circle cx="110" cy="155" r="6" fill="#F4D070" opacity="0.7"/>
        <circle cx="125" cy="165" r="4" fill="#E8C040" opacity="0.6"/>
        <circle cx="140" cy="170" r="3" fill="#F4D070" opacity="0.5"/>
        {/* Leaves */}
        <ellipse cx="160" cy="155" rx="14" ry="8" fill="#5A8A50" transform="rotate(20,160,155)"/>
        <ellipse cx="170" cy="168" rx="12" ry="7" fill="#4A7A40" transform="rotate(-10,170,168)"/>
      </>}
      {pos==='bl' && <>
        {/* Cardamom pods */}
        <rect x="20" y="120" width="12" height="24" rx="6" fill="#7A9A60" transform="rotate(-15,26,132)"/>
        <rect x="38" y="130" width="12" height="22" rx="6" fill="#6A8A50" transform="rotate(10,44,141)"/>
        {/* Pepper scatter */}
        <circle cx="65" cy="140" r="5" fill="#2B2B2B" opacity="0.5"/>
        <circle cx="55" cy="155" r="4" fill="#2B2B2B" opacity="0.4"/>
        <circle cx="75" cy="158" r="3.5" fill="#C04A28" opacity="0.5"/>
        {/* Orange dot */}
        <circle cx="30" cy="168" r="6" fill="#F79B3D" opacity="0.55"/>
        <circle cx="12" cy="140" r="4" fill="#F79B3D" opacity="0.4"/>
      </>}
    </svg>
  );
}
...
export default function HomePage() {
  return (
    <>
      ...
      <div style={{minHeight:'100vh',background:'#FAF7F2',fontFamily:'Inter,sans-serif',color:'#2B2B2B',overflowX:'hidden',position:'relative'}}>
        <style>{`
          *{box-sizing:border-box;margin:0;padding:0} a{text-decoration:none}
          ...
          .nav{
            ...
            border:1.5px solid rgba(247,155,61,0.2);border-radius:50px;
            padding:8px 10px 8px 20px;display:flex;align-items:center;gap:2px;
            box-shadow:0 6px 28px rgba(0,0,0,0.08),inset 0 1px 0 rgba(255,255,255,0.8);
            width:auto;white-space:nowrap;
          }
          .nlnk{font-size:14px;color:#7A7A7A;font-weight:500;transition:color 0.15s;padding:6px 14px;border-radius:30px;}
          .nlnk:hover{color:#2B2B2B;background:rgba(247,155,61,0.08);}

          /* ── Cards ── */
          /* Tastywala white card */
          .tw-card{
          ...
          .tw-card:hover{transform:translateY(-6px);box-shadow:0 16px 48px rgba(0,0,0,0.12);}

          /* Clay card — retained for hero/art areas */
          .cc{
            background:rgba(255,252,245,0.88);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);
            border:1.5px solid rgba(247,155,61,0.2);border-radius:22px;
            box-shadow:0 6px 24px rgba(0,0,0,0.08),inset 0 1px 0 rgba(255,255,255,0.8);
            transition:all 0.22s cubic-bezier(0.34,1.56,0.64,1);
          }
          .cc:hover{transform:translateY(-5px);box-shadow:0 18px 44px rgba(0,0,0,0.13);}

          /* Plan card */
          .pc{
            background:#FFFFFF;border-radius:20px;padding:32px;position:relative;
            border:1.5px solid #F0EBE3;
            box-shadow:0 4px 24px rgba(0,0,0,0.07);
            transition:all 0.22s cubic-bezier(0.34,1.56,0.64,1);
          }
          .pc.pop{border-color:rgba(247,155,61,0.55);box-shadow:0 12px 40px rgba(247,155,61,0.18);}
          .pc:hover{transform:translateY(-5px);box-shadow:0 16px 48px rgba(0,0,0,0.12);}
          .pc.pop:hover{box-shadow:0 20px 52px rgba(247,155,61,0.25);}

          /* ── Pill buttons ── */
          .btn-primary{
            background:#F79B3D;color:#fff;border:none;border-radius:50px;
            font-family:Poppins,sans-serif;font-weight:700;cursor:pointer;
            transition:all 0.2s;box-shadow:0 6px 20px rgba(247,155,61,0.38);
          }
          .btn-primary:hover{background:#F48A1E;transform:translateY(-2px);box-shadow:0 10px 28px rgba(247,155,61,0.48);}

          .btn-outline{
            background:#fff;color:#F79B3D;border:2px solid #F79B3D;border-radius:50px;
            font-weight:700;cursor:pointer;transition:all 0.2s;font-family:Poppins,sans-serif;
          }
          .btn-outline:hover{background:#F79B3D;color:#fff;transform:translateY(-2px);}

          .btn-dark{
            background:#1E1B18;color:#FFF5E8;border:none;border-radius:50px;
            font-family:Poppins,sans-serif;font-weight:700;cursor:pointer;
            transition:all 0.2s;box-shadow:0 6px 20px rgba(30,27,24,0.25);
          }
          ...
          .plan-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;max-width:940px;margin:0 auto;}
          ...
          .step-num{
            ...
            box-shadow:0 4px 14px rgba(247,155,61,0.38);
          }

          /* Category chip */
          .chip{padding:8px 22px;border-radius:50px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;border:2px solid #F79B3D;}
          .chip.on{background:#F79B3D;color:#fff;box-shadow:0 4px 12px rgba(247,155,61,0.3);}
          .chip.off{background:#fff;color:#F79B3D;}
          .chip.off:hover{background:rgba(247,155,61,0.08);}
        ...
        <nav className="nav">
          ...
          <Link href="/admin/login" style={{marginLeft:6,padding:'9px 22px',background:'#F79B3D',color:'#fff',borderRadius:30,fontSize:13,fontWeight:700,boxShadow:'0 4px 14px rgba(247,155,61,0.35)'}}>
            Get Started
          </Link>
        </nav>

        {/* ══ HERO ══ */}
        <section style={{paddingTop:112,paddingBottom:40,position:'relative',zIndex:1}}>
          {/* Tastywala-style soft background blob */}
          <div style={{position:'absolute',top:'5%',right:'5%',width:520,height:520,borderRadius:'50%',background:'radial-gradient(circle,#F5E6D3 0%,transparent 70%)',pointerEvents:'none',zIndex:0}}/>
          <div style={{position:'absolute',bottom:'10%',left:'-5%',width:300,height:300,borderRadius:'50%',background:'radial-gradient(circle,rgba(247,155,61,0.1) 0%,transparent 70%)',pointerEvents:'none',zIndex:0}}/>
          {/* Spice corner */}
          <div style={{position:'absolute',top:80,right:0,pointerEvents:'none'}}>
            <SpiceDecor pos="tr" opacity={0.5}/>
          </div>
          ...
          <div className="inner" style={{position:'relative',zIndex:1}}>
            <div className="split">
              ...
              <div style={{animation:'fadeUp 0.8s ease forwards'}}>
                {/* Badge pill */}
                <div style={{display:'inline-flex',alignItems:'center',gap:7,padding:'7px 16px',borderRadius:50,background:'rgba(247,155,61,0.1)',border:'1.5px solid rgba(247,155,61,0.3)',fontSize:12,fontWeight:700,color:'#C07020',marginBottom:24,letterSpacing:'0.02em'}}>
                  <span style={{width:7,height:7,borderRadius:'50%',background:'#F79B3D',animation:'blink 2s infinite'}}/>
                  AR-Powered Restaurant Menus
                </div>

                <h1 style={{fontFamily:'Poppins,sans-serif',fontWeight:900,fontSize:'clamp(38px,4.5vw,64px)',lineHeight:1.05,letterSpacing:'-0.03em',color:'#1E1B18',marginBottom:20}}>
                  Your menu,<br/>
                  <span style={{background:'linear-gradient(135deg,#F79B3D,#E05A3A)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>alive in 3D</span>
                </h1>

                <p style={{fontSize:17,color:'#7A7A7A',lineHeight:1.75,marginBottom:36,maxWidth:420}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:40,flexWrap:'wrap'}}>
                  <Link href="/admin/login">
                    <button className="btn-primary" style={{padding:'14px 30px',fontSize:15}}>Get Started Free</button>
                  </Link>
                  <a href="#how">
                    <button className="btn-outline" style={{padding:'12px 26px',fontSize:15}}>See how it works</button>
                  </a>
                </div>

                {/* Stats row */}
                <div style={{display:'flex',alignItems:'center',gap:20,paddingTop:20,borderTop:'1px solid #E6E6E6'}}>
                  {[
                    {val:'500+',  lbl:'Restaurants'},
                    {val:'50k+',  lbl:'AR Views / month'},
                    {val:'4.9★',  lbl:'Avg rating'},
                  ].map((s,i) => (
                    <div key={s.val} style={{display:'flex',alignItems:'center',gap:20}}>
                      {i>0 && <div style={{width:1,height:32,background:'#E6E6E6'}}/>}
                      <div style={{textAlign:'center'}}>
                        <div style={{fontFamily:'Poppins,sans-serif',fontWeight:800,fontSize:22,color:'#F79B3D'}}>{s.val}</div>
                        <div style={{fontSize:12,color:'#7A7A7A',marginTop:2}}>{s.lbl}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* RIGHT — 3D Clay Food Scene (preserved exactly) */}
              <div style={{position:'relative',animation:'fadeUp 1s ease 0.15s both'}}>
                <div style={{position:'absolute',top:'8%',left:'8%',right:'8%',bottom:'5%',background:'radial-gradient(ellipse,rgba(245,230,211,0.8),transparent 68%)',filter:'blur(18px)',borderRadius:'50%',pointerEvents:'none'}}/>
                <svg viewBox="0 0 580 500" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',height:'auto',filter:'drop-shadow(0 20px 44px rgba(120,70,30,0.15))',position:'relative'}}>
                  <defs>
                  ...
                  <circle cx="100" cy="78" r="4.5" fill="#F79B3D" opacity="0.72"/>
                  <circle cx="106" cy="71" r="2.8" fill="#F79B3D" opacity="0.48"/>
                  ...
                  <g transform="translate(546,98)" fill="#F79B3D" opacity="0.68"><polygon points="0,-7 1.8,-1.8 7,0 1.8,1.8 0,7 -1.8,1.8 -7,0 -1.8,-1.8"/></g>
        ...
        <section id="how" className="section" style={{background:'#F5E6D3',position:'relative',overflow:'hidden'}}>
          <SpiceDecor pos="tl" opacity={0.4}/>
          <SpiceDecor pos="br" opacity={0.35}/>
          <div className="inner" style={{position:'relative',zIndex:1}}>
            <div className="split">
              <div>
                <div style={{fontSize:12,fontWeight:700,color:'#F79B3D',letterSpacing:'0.12em',textTransform:'uppercase',marginBottom:12}}>How it works</div>
                ...
                <p style={{fontSize:16,color:'#7A7A7A',lineHeight:1.75,marginBottom:32,maxWidth:400}}>
                  ...
                  {[
                    ...
                    <div key={s.n} style={{display:'flex',gap:16,alignItems:'flex-start'}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:14,color:'#7A7A7A',lineHeight:1.65}}>{s.d}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Analytics scene SVG — preserved */}
              <div style={{position:'relative'}}>
                <div style={{position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 50%,rgba(255,255,255,0.4),transparent 70%)',borderRadius:'50%',pointerEvents:'none'}}/>
                <svg viewBox="0 0 480 420" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',height:'auto',filter:'drop-shadow(0 16px 36px rgba(120,70,30,0.14))'}}>
                  <defs>
                  ...
                  <rect x="198" y="130" width="14" height="63" rx="4" fill="#F79B3D" opacity="0.85"/>
                  ...
                  <rect x="252" y="120" width="14" height="73" rx="4" fill="#F79B3D" opacity="0.92"/>
                  ...
                  <rect x="158" y="102" width="58" height="24" rx="6" fill="rgba(247,155,61,0.25)"/>
                  ...
                  <rect x="32" y="155" width="110" height="68" rx="14" fill="rgba(255,248,232,0.95)" filter="url(#ds2)"/>
                  <rect x="42" y="164" width="22" height="22" rx="8" fill="rgba(247,155,61,0.2)"/>
                  ...
                  <text x="72" y="187" fill="#F79B3D" fontSize="14" fontWeight="800" fontFamily="Poppins,sans-serif">12,450</text>
                  <text x="72" y="198" fill="rgba(42,31,16,0.45)" fontSize="8.5" fontFamily="Inter,sans-serif">this month</text>
                  <rect x="338" y="148" width="110" height="68" rx="14" fill="rgba(255,248,232,0.95)" filter="url(#ds2)"/>
                  ...
                  <text x="378" y="180" fill="#F79B3D" fontSize="14" fontWeight="800" fontFamily="Poppins,sans-serif">3,291</text>
                  ...
                  <g transform="translate(442,90)" fill="#F79B3D" opacity="0.65"><polygon points="0,-6 1.5,-1.5 6,0 1.5,1.5 0,6 -1.5,1.5 -6,0 -1.5,-1.5"/></g>
        ...
        {/* ══ FEATURES ══ — white background, Tastywala white cards */}
        <section id="features" className="section" style={{background:'#FFFFFF',position:'relative',overflow:'hidden'}}>
          <SpiceDecor pos="tr" opacity={0.3}/>
          <SpiceDecor pos="bl" opacity={0.28}/>
          <div className="inner" style={{position:'relative',zIndex:1}}>
            <div style={{textAlign:'center',marginBottom:56}}>
              <div style={{fontSize:12,fontWeight:700,color:'#F79B3D',letterSpacing:'0.12em',textTransform:'uppercase',marginBottom:10}}>Features</div>
              ...
              <div style={{width:48,height:4,borderRadius:2,background:'#F79B3D',margin:'0 auto 16px'}}/>
              <p style={{fontSize:16,color:'#7A7A7A',maxWidth:480,margin:'0 auto'}}>One platform to manage your AR menu, track analytics, and grow your restaurant.</p>
            </div>
            <div className="feat-grid">
              {[
                {title:'AR Menu Viewer',  desc:'Dishes appear life-size in 3D on your customers\' table. Powered by WebAR.',      bg:'rgba(90,138,176,0.1)',   scene:<ARScene/>},
                {title:'Live Analytics',  desc:'Track scans, AR views, popular dishes, and repeat visitors in real time.',         bg:'rgba(42,58,90,0.07)',    scene:<AnalyticsScene/>},
                {title:'Instant QR Code', desc:'A unique QR code for your restaurant is generated the moment you sign up.',         bg:'rgba(196,181,212,0.15)', scene:<QRScene/>},
                {title:'Your Subdomain',  desc:'Your restaurant gets its own URL — restaurantname.advertradical.com.',             bg:'rgba(247,155,61,0.1)',   scene:<LinkScene/>},
                {title:'Offers & Promos', desc:'Push time-limited promotional banners to your live menu in one click.',            bg:'rgba(247,155,61,0.08)',  scene:<PromoScene/>},
                ...
                <div key={f.title} className="tw-card" style={{padding:'24px 22px',display:'flex',flexDirection:'column',gap:14}}>
                  <div style={{width:'100%',height:96,borderRadius:14,background:f.bg,overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
                    {f.scene}
                  </div>
                  <div style={{width:32,height:3,borderRadius:2,background:'#F79B3D'}}/>
                  <h3 style={{fontFamily:'Poppins,sans-serif',fontWeight:700,fontSize:15,color:'#1E1B18'}}>{f.title}</h3>
                  <p style={{fontSize:13.5,color:'#7A7A7A',lineHeight:1.65,marginTop:-4}}>{f.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ══ PLANS ══ — beige background */}
        <section id="plans" className="section" style={{background:'#F5E6D3',position:'relative',overflow:'hidden'}}>
          <SpiceDecor pos="tl" opacity={0.35}/>
          <SpiceDecor pos="br" opacity={0.3}/>
          <div className="inner" style={{position:'relative',zIndex:1}}>
            <div style={{textAlign:'center',marginBottom:56}}>
              <div style={{fontSize:12,fontWeight:700,color:'#F79B3D',letterSpacing:'0.12em',textTransform:'uppercase',marginBottom:10}}>Pricing</div>
              ...
              <div style={{width:48,height:4,borderRadius:2,background:'#F79B3D',margin:'0 auto 16px'}}/>
              <p style={{fontSize:16,color:'#7A7A7A'}}>6-month plans. No hidden fees. Cancel anytime.</p>
            </div>
            <div className="plan-grid">
              {plans.map(p=>(
                <div key={p.name} className={`pc${p.tag==='Popular'?' pop':''}`}>
                  {p.tag && (
                    <div style={{position:'absolute',top:-15,left:'50%',transform:'translateX(-50%)',padding:'5px 18px',background:'#F79B3D',color:'#fff',fontSize:11,fontWeight:700,borderRadius:50,whiteSpace:'nowrap',boxShadow:'0 4px 14px rgba(247,155,61,0.38)'}}>
                      ✦ {p.tag}
                    </div>
                  )}
                  {/* Plan illustration */}
                  <div style={{width:'100%',height:88,borderRadius:16,marginBottom:20,background:p.tag==='Popular'?'rgba(247,155,61,0.1)':'#FAF7F2',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
                  ...
                  <div style={{display:'flex',alignItems:'baseline',gap:4,marginBottom:20}}>
                    <span style={{fontFamily:'Poppins,sans-serif',fontWeight:900,fontSize:32,color:p.tag==='Popular'?'#F79B3D':'#1E1B18'}}>{p.price}</span>
                    <span style={{fontSize:13,color:'#7A7A7A'}}>{p.per}</span>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:24}}>
                    {[`${p.items} AR menu items`,`${p.storage} storage`,'Analytics dashboard','Custom QR code','Your own subdomain'].map(f=>(
                      <div key={f} style={{display:'flex',alignItems:'center',gap:9,fontSize:13.5,color:'#7A7A7A'}}>
                        <span style={{width:18,height:18,borderRadius:50,background:p.tag==='Popular'?'#F79B3D':'rgba(247,155,61,0.15)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,color:p.tag==='Popular'?'#fff':'#F79B3D',fontWeight:800,flexShrink:0}}>✓</span>
                        {f}
                      </div>
                    ))}
                  </div>
                  <button
                    className={p.tag==='Popular'?'btn-primary':'btn-outline'}
                    style={{width:'100%',padding:'13px',borderRadius:50,fontSize:14,fontFamily:'Poppins,sans-serif',fontWeight:700}}>
                    Get Started
                  </button>
                </div>
              ))}
        ...
        {/* ══ CTA ══ */}
        <section className="section" style={{background:'#FAF7F2',position:'relative',overflow:'hidden'}}>
          <SpiceDecor pos="tr" opacity={0.3}/>
          <SpiceDecor pos="bl" opacity={0.28}/>
          <div className="inner" style={{position:'relative',zIndex:1}}>
            <div style={{
              ...
              boxShadow:'0 20px 60px rgba(30,27,24,0.2)',
              overflow:'hidden',position:'relative',
            }}>
              {/* Subtle orange glow */}
              <div style={{position:'absolute',top:'-20%',right:'-5%',width:400,height:400,borderRadius:'50%',background:'radial-gradient(circle,rgba(247,155,61,0.15) 0%,transparent 70%)',pointerEvents:'none'}}/>
              <div className="split" style={{gap:40}}>
                <div style={{position:'relative',zIndex:1}}>
                  <h2 style={{fontFamily:'Poppins,sans-serif',fontWeight:900,fontSize:'clamp(28px,3.5vw,46px)',color:'#FFF5E8',lineHeight:1.1,marginBottom:16}}>
                    Ready to bring your<br/>
                    <span style={{background:'linear-gradient(135deg,#F79B3D,#F4C06A)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>menu to life?</span>
                  </h2>
                  <p style={{fontSize:16,color:'rgba(255,245,232,0.6)',lineHeight:1.7,marginBottom:32,maxWidth:380}}>
                    Join hundreds of restaurants already using Advert Radical to delight their customers with AR dining.
                  </p>
                  <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
                    <Link href="/admin/login">
                      <button className="btn-primary" style={{padding:'15px 32px',fontSize:15}}>Start Free Today</button>
                    </Link>
                    <a href="mailto:hello@advertradical.com">
                      <button style={{padding:'13px 26px',background:'rgba(255,255,255,0.08)',color:'#FFF5E8',border:'1.5px solid rgba(255,255,255,0.18)',borderRadius:50,fontSize:15,fontWeight:700,fontFamily:'Poppins,sans-serif',cursor:'pointer',transition:'all 0.2s'}}
        ...
        {/* ══ FOOTER ══ */}
        <footer style={{borderTop:'1px solid #E6E6E6',padding:'28px 0',background:'#FFFFFF'}}>
          <div className="inner" style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:16}}>
            ...
            <span style={{fontSize:13,color:'#7A7A7A'}}>© {new Date().getFullYear()} Advert Radical. All rights reserved.</span>
            <div style={{display:'flex',gap:24}}>
              {['Privacy','Terms'].map(l=><a key={l} href="#" style={{fontSize:13,color:'#7A7A7A',transition:'color 0.15s'}} onMouseOver={e=>e.currentTarget.style.color='#F79B3D'} onMouseOut={e=>e.currentTarget.style.color='#7A7A7A'}>{l}</a>)}
              <Link href="/admin/login" style={{fontSize:13,color:'#7A7A7A'}}>Sign in</Link>
              <Link href="/superadmin/login" style={{fontSize:13,color:'#7A7A7A'}}>Super Admin</Link>
            </div>
          </div>
        </footer>
      </div>
...
/* ── Feature card SVG illustrations — all preserved ── */
function ARScene() {
  return (
    <svg viewBox="0 0 200 100" style={{width:'100%',height:'100%'}}>
      ...
      <circle cx="164" cy="28" r="5" fill="#F79B3D" opacity="0.65"/>
    </svg>
  );
}
...
function AnalyticsScene() {
  return (
    <svg viewBox="0 0 200 100" style={{width:'100%',height:'100%'}}>
      ...
      <rect x="30" y="17" width="42" height="14" rx="5" fill="rgba(247,155,61,0.35)"/>
      ...
      <rect x="32" y="21" width="10" height="5" rx="1.5" fill="#F79B3D" opacity="0.9"/>
      ...
      <rect x="76"  y="36" width="14" height="44" rx="5" fill="#F79B3D"/>
      <rect x="96"  y="50" width="14" height="30" rx="5" fill="#F4D070"/>
      <rect x="116" y="40" width="14" height="40" rx="5" fill="#C4B5D4"/>
      <rect x="136" y="28" width="14" height="52" rx="5" fill="#F79B3D" opacity="0.85"/>
...
function QRScene() {
  return (
    <svg viewBox="0 0 200 100" style={{width:'100%',height:'100%'}}>
      ...
      <rect x="74" y="80" width="52" height="8" rx="4" fill="rgba(247,155,61,0.25)"/>
      <circle cx="38" cy="40" r="9" fill="#F79B3D" opacity="0.3"/>
...
function LinkScene() {
  return (
    <svg viewBox="0 0 200 100" style={{width:'100%',height:'100%'}}>
      ...
      <rect x="25" y="38" width="66" height="24" rx="12" fill="rgba(247,155,61,0.15)"/>
      <rect x="30" y="47" width="10" height="6" rx="2" fill="rgba(200,120,40,0.55)"/>
      <rect x="44" y="47" width="40" height="6" rx="2" fill="rgba(200,120,40,0.4)"/>
      ...
      <circle cx="188" cy="38" r="7" fill="#F79B3D" opacity="0.6"/>
...
function PromoScene() {
  return (
    <svg viewBox="0 0 200 100" style={{width:'100%',height:'100%'}}>
      <rect x="18" y="22" width="164" height="56" rx="16" fill="rgba(247,155,61,0.12)"/>
      <rect x="28" y="32" width="144" height="36" rx="10" fill="rgba(247,155,61,0.18)"/>
      <rect x="42" y="40" width="14" height="20" rx="3" fill="#F79B3D" opacity="0.7"/>
      <polygon points="56,38 74,28 74,62 56,52" fill="#F79B3D" opacity="0.75"/>
      <rect x="74" y="40" width="6" height="20" rx="3" fill="#F79B3D" opacity="0.55"/>
      <circle cx="98" cy="44" r="4" fill="#F4D070" opacity="0.85"/>
      <circle cx="110" cy="58" r="3" fill="#F4D070" opacity="0.7"/>
      <rect x="122" y="40" width="36" height="7" rx="3" fill="rgba(247,155,61,0.35)"/>
      <rect x="120" y="52" width="30" height="6" rx="3" fill="rgba(247,155,61,0.2)"/>
...
function NoAppScene() {
  return (
    <svg viewBox="0 0 200 100" style={{width:'100%',height:'100%'}}>
      ...
      <circle cx="60" cy="42" r="12" fill="rgba(247,155,61,0.12)" stroke="#F79B3D" strokeWidth="1.5"/>
      <line x1="55" y1="37" x2="65" y2="47" stroke="#F79B3D" strokeWidth="2.2" strokeLinecap="round"/>
      <line x1="65" y1="37" x2="55" y2="47" stroke="#F79B3D" strokeWidth="2.2" strokeLinecap="round"/>
      <rect x="34" y="58" width="52" height="7" rx="3" fill="rgba(100,60,30,0.15)"/>
      <line x1="94" y1="50" x2="104" y2="50" stroke="rgba(247,155,61,0.5)" strokeWidth="2" strokeDasharray="3,2"/>
      <polygon points="104,46 110,50 104,54" fill="rgba(247,155,61,0.5)"/>
      <circle cx="18" cy="35" r="6" fill="#8FC4A8" opacity="0.55"/>
      <circle cx="186" cy="30" r="7" fill="#F79B3D" opacity="0.6"/>
    </svg>
  );
}
...
function PlanIllustration({ name }) {
  if (name === 'Basic') return (
    <svg viewBox="0 0 260 80" style={{width:'100%',height:'100%'}}>
      <rect x="18" y="16" width="48" height="48" rx="14" fill="rgba(247,155,61,0.12)"/>
      <circle cx="42" cy="40" r="16" fill="rgba(255,230,170,0.9)"/>
      <circle cx="42" cy="40" r="10" fill="rgba(255,215,140,0.8)"/>
      <circle cx="42" cy="40" r="5" fill="#F79B3D" opacity="0.7"/>
      ...
      <rect x="82" y="20" width="1.5" height="40" rx="0.75" fill="rgba(247,155,61,0.15)"/>
      <text x="98" y="36" fill="#C07020" fontSize="13" fontWeight="700" fontFamily="Poppins,sans-serif">10 AR items</text>
      <text x="98" y="54" fill="#7A7A7A" fontSize="11" fontFamily="Inter,sans-serif">500MB storage</text>
    </svg>
  );
  if (name === 'Pro') return (
    <svg viewBox="0 0 260 80" style={{width:'100%',height:'100%'}}>
      <rect x="12" y="16" width="44" height="48" rx="13" fill="rgba(247,155,61,0.1)"/>
      ...
      <rect x="60" y="18" width="40" height="44" rx="12" fill="rgba(247,155,61,0.08)"/>
      ...
      <rect x="114" y="18" width="1.5" height="44" rx="0.75" fill="rgba(247,155,61,0.15)"/>
      <text x="128" y="36" fill="#C07020" fontSize="13" fontWeight="700" fontFamily="Poppins,sans-serif">40 AR items</text>
      <text x="128" y="54" fill="#7A7A7A" fontSize="11" fontFamily="Inter,sans-serif">2GB storage</text>
    </svg>
  );
  return (
    <svg viewBox="0 0 260 80" style={{width:'100%',height:'100%'}}>
      <rect x="8" y="18" width="36" height="44" rx="11" fill="rgba(196,181,212,0.2)"/>
      ...
      <rect x="48" y="18" width="36" height="44" rx="11" fill="rgba(196,181,212,0.15)"/>
      ...
      <rect x="88" y="18" width="36" height="44" rx="11" fill="rgba(196,181,212,0.1)"/>
      ...
      <rect x="136" y="18" width="1.5" height="44" rx="0.75" fill="rgba(247,155,61,0.15)"/>
      <text x="150" y="36" fill="#6A4A8A" fontSize="13" fontWeight="700" fontFamily="Poppins,sans-serif">100 AR items</text>
      <text x="150" y="54" fill="#7A7A7A" fontSize="11" fontFamily="Inter,sans-serif">5GB storage</text>
    </svg>
  );
}
...
function CelebrationScene() {
  return (
    <svg viewBox="0 0 340 240" xmlns="http://www.w3.org/2000/svg" style={{width:320,height:220,filter:'drop-shadow(0 8px 24px rgba(0,0,0,0.1))'}}>
      <defs>
      ...
      <circle cx="128" cy="126" r="5" fill="#F79B3D" opacity="0.75"/>
      ...
      <rect x="84" y="112" width="18" height="8" rx="3" fill="rgba(247,155,61,0.35)"/>
      ...
      {/* Confetti — amber theme */}
      <rect x="40" y="60" width="8" height="8" rx="2" fill="#F79B3D" opacity="0.7" transform="rotate(25,44,64)"/>
      ...
      <circle cx="55" cy="85" r="4" fill="#F79B3D" opacity="0.65"/>
      <circle cx="295" cy="72" r="3.5" fill="#F4A0B0" opacity="0.6"/>
      <g transform="translate(315,105)" fill="#F79B3D" opacity="0.7"><polygon points="0,-6 1.5,-1.5 6,0 1.5,1.5 0,6 -1.5,1.5 -6,0 -1.5,-1.5"/></g>
      <g transform="translate(28,155)" fill="#8FC4A8" opacity="0.65"><polygon points="0,-5 1.2,-1.2 5,0 1.2,1.2 0,5 -1.2,1.2 -5,0 -1.2,-1.2"/></g>
      <circle cx="170" cy="42" r="5" fill="#F79B3D" opacity="0.45"/>
